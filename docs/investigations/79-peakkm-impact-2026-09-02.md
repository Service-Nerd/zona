...
2. **Phase 1 (returning runner) is incomplete.** The secondary quality slot filters rows by `FITNESS_RANK[row.fitness_level_min] <= structural`. Most quality rows in `sessionCatalogueData.ts` are `fitness_level_min: 'intermediate'`, so a returning runner (structural `beginner`) can select almost nothing for slot 2. **Descoped to its own board item** — it changes prescription for runners who never touched the control (Willy's condition).
