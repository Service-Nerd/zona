...
3. **How many other users are affected?** 13 rows in `plans`. I did not audit the other 12 — out of scope under constraint 7, and most are likely founder/test accounts. **A count of production plans where `hr_zone_method = 'karvonen'` and `max_hr` deviates >15% from Tanaka is the single most useful next query**, and needs no per-user data.
