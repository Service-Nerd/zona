...
> **Rows 14/15 — `long_run_with_segment` is the display gate, and the split is §16's (RACE-PACE-OVERLAY-REACH-01, Coaching Board 2026-09-14).** `composeSession` used to decide whether a long run carried a race-pace segment by asking whether its NAME contained "marathon-pace". Row 15 is named "Marathon-pace long run" and row 14 is not, so **HM rendered the overlay on 0 of 18 peak sessions** while MARATHON rendered 12 of 12. The gate is now `main_set_structure.type === 'long_run_with_segment'` — these two rows and no others. Both rows also declared their own `easy_pct`/`race_pace_pct` (65/35 and 60/40) against §16's 20%, read by nothing; the board ruled §16 governs and those two fields were **deleted** rather than left as a second, contradicting declaration (§17). `race_pace_zone` stays and is now authority — the session card quotes it, so an HM runner is told "HM target", not "MP target".
...
| 28 | `tenk_race_simulation` | 10K-pace race simulation | race_specific | peak | 10K | intermediate | T4 | **v2** · reps × (2000m at goal pace / 90s jog) · `scaling: 'reps'`. **CAT-10K-RACE-SPECIFIC-01, Coaching Board 2026-09-11** — 10K's SECOND race-specific row. Measured before the sitting: 100% of 96 generated 10K time-target plans placed `tenk_pace_intervals` TWICE and no plan saw two different race-specific sessions, while §93 provisions up to three race-specific peak slots. Differs on the axis that carries 10K specificity — time at goal pace per rep against a shorter recovery (~6km at pace in three efforts vs ~4.8km in four). NOT a duplicate of `threshold_mile_repeats` / `tempo_cruise_short`: those are anchored at **threshold**, this at **goal**. Peak-only and intermediate-minimum on Willy's guard — 90s recovery on 2km reps is a real step up in continuous load. |
| 29 | `mp_blocks` | Marathon-pace blocks | race_specific | peak | MARATHON | intermediate | T4 | **v2** · reps × (4000m at goal pace / 3 min jog) · `scaling: 'reps'`. **CAT-MARATHON-RACE-SPECIFIC-01, Coaching Board 2026-09-11 (§105)** — MARATHON's SECOND race-specific row, and the first that is not a long run. Measured before: 100% of 96 marathon time-target plans reused `mp_long_run`, up to THREE times in one plan, so every scrap of marathon-specific work lived inside the long run. Separating goal-pace exposure from long-run day means MP work survives a week the long run does not. Anchored at **goal** (MP), not T — distinct from the threshold rows beside it. Named in the "…-pace…" family per §104: `Session.stimulus` is stamped from the label, so a race_specific row named outside it is invisible to §22's own check. |
