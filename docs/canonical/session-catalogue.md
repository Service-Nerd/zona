...
| 8 | `threshold_ladder` | Threshold ladder | threshold | build, peak | 10K, HM, MARATHON, 50K, 100K | intermediate | T3 | **v2** · 3-5-8-5-3 min at T, 90s jogged recovery between · `scaling: fixed` — the ladder's shape IS the session, NOT structure-driven sized (still the flat quality-session formula). Audit §E.5, unblocked by v2 case 1. Eligible at `weeklyKm >= 45` OR a sustained recent threshold pattern (§53, Coaching Board 2026-09-03) — see `THRESHOLD_LADDER_MIN_WEEKLY_KM`/`THRESHOLD_LADDER_ALT_*`. "Fitness ≥" column corrected from `experienced` (stale — CAT-ULTRA-THIN-01, 2026-08-21, replaced the fitness-label gate with the volume floor itself; this table wasn't regenerated). |
