...
| DIV-022 | `components/marketing/SiteFooter.tsx` | **Voice moment demoted to chrome.** The shared footer rendered `BRAND.brandStatement` on all 8 marketing pages, duplicating it on two: the homepage (which ends on a designed 48px closing moment) and `/privacy` (which already carries its own 10px line — its documented home). Introduced by GTM-SITE-01's own standardisation. DIV-020 class. | ✅ Fixed 2026-09-10 — removed from the shared footer; footer is now navigation + legal only. Homepage closing moment and the `/privacy` line both retained, untouched. |
