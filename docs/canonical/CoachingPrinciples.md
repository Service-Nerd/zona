...
>
> Enforced hard by **`INV-PLAN-VO2MAX-MAIN-SET-CAP`** (error, now the work-minute band). The relative ordering **`INV-PLAN-MAIN-SET-ORDERING`** stays `warn` and compares VO2max on its WORK minutes (its main set carries recovery the sustainable categories don't).
>
...
>
> **Promotion attempted and reverted — Coaching Board, 2026-09-03, SIZING-REALLOC-01 still open.** The board ruled CORRECT-contingent on promoting `warn` → `error`, conditional on the real test suite staying clean — measurement had shown 0 violations across 5,000 plans weighted toward the 15–40km/week risk band, plus 84 plans built to exercise the three rows the same-day sizing generalisation didn't reach (`tempo_cruise`, `tempo_continuous`, `goal_pace_sharpener`). The contingency did its job: `npx vitest run` immediately failed an existing test (`trainingDayFloor.test.ts`) on a case the measurement sample never hit — `experienced` fitness at 12km/week, 7 days available. VO2max's main set (18 min, an absolute band that doesn't shrink) exceeded `tempo_cruise`'s main set (8 min, still sized by the flat `QUALITY_SESSION_PCT_OF_WEEKLY` formula, which does shrink at very low absolute volume). **The asymmetry the board thought it had closed only closes for the three rows migrated today — it is still open for the three that weren't.** Severity stays `warn`. SIZING-REALLOC-01 closes for real once `tempo_cruise`, `tempo_continuous`, and `goal_pace_sharpener` also migrate to structure-driven sizing — a defined, not a vague, remaining task. Willy's CD-14 amendment 2 (a rep-count ceiling per category, with the budget overflowing into rep *length*) is **blocked on SC-08** — reps do not reach the plan at all today, so a ceiling on them cannot be expressed or checked.
>
...
> **Ruled on, INSUFFICIENT EVIDENCE — Coaching Board, 2026-09-03.** Measured (5,000-plan sample): 49,551 week-rows sit below the flat 45km/week gate, and among them threshold-category work reaches as much as 36.3% of that week's total training minutes — a genuinely threshold-committed runner can still be denied the ladder purely on total volume. Median share in ineligible weeks is 0.0% (most sub-floor weeks have no threshold work at all), so this is a real tail case, not the norm. **Recorded disagreement, not resolved:** Willy holds the volume floor is also a tissue/aerobic-base minimum, and a pure share-based replacement risks admitting a runner with the intensity habit but not the structural base to absorb this session. Sims holds a volume-only gate is the same male-mileage-default pattern she flags generally, and that a sustained pattern of threshold work at lower volume is evidence of readiness, not a red flag. **What would settle it:** a proposal for a second eligibility path (existing floor OR a specific, pre-computable relative measure — not a same-week circular one) satisfying both concerns, PLUS a check of whether simply lowering the flat 45km number (McMillan) closes most of the gap without new complexity. Do not invent a formula outside a board sitting.
