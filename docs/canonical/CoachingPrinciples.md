...
**Run cadence — committed run days only.** The block runs the same number of days/week the athlete *actually* trained, derived from the completed plan (`inferRunDaysPerWeek`), never from `meta.days_available` (which counts strength/cross-train days and may be aspirational). Two rules keep it conservative, matching the tick-over ethos: (1) **recovery jogs don't count** — a recovery run is supplemental easy volume the engine layers on, not a committed run day, so an ultra week of 3 committed runs + 1 recovery jog reads as 3, not 4; (2) when a plan's run-days drift across weeks, the **lower median** wins — maintain at the sustainable cadence, not the busiest build week. A maintenance week may never schedule more run days than this source cadence. `INV-MAINT-CADENCE`.
...
**Surfacing — announce, never gate (MAINT-04).** The block is auto-live the moment the race result is logged; the runner never *approves* it (an accept/decline gate would hand an overtrained runner a way to decline their own recovery — SLT-rejected). But it must not appear silently: a one-time Today announcement marks the race done, explains in one §75 sentence why the plan eased, and shows the shape (days/week · weeks · below base). Its affordance is "See the plan" (→ adjust on the Plan screen), not accept/decline — editable, not approvable. On the Plan screen the block reads as its own "After the race" chapter (recovery-green seam + "Maintenance N" labels), not a continuation of the race plan's week numbering. Seen state lives on `result_embedded.maintenance_transition_seen` (self-keyed per race). Rule-engine copy carries no AIMark; the PAID weekly debrief does.
