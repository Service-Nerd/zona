...
**The honest bit:** I verified the login change at 375px in a real browser and watched it render. I could **not** verify the three onboarding screens — they sit behind auth, and there is no fixture page for the wizard the way there is for Me and Coach. That is the same gap that once let me ship a comment describing an arc I had never built. The code is right and the tests pass; the pixels are unseen, and the founder should look before this goes to TestFlight.
