...
**Shipped:** All 106 coaching principles are mapped to an invariant, a named test, or a reasoned exemption, and the build fails if that stops being true. Unverified debt went 29 → 0 across five categories.
...
**Hook material:** 106 principles. 29 with no check. Zero now: 71 invariants, 29 named tests, 6 exemptions with written reasons ≥30 characters, enforced by a test that fails if you add a §107 without deciding how it is checked. And one test that existed, passed the gate, and had never run.
**Postable?:** yes
