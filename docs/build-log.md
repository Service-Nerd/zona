...
**Round 2 + 3 learning:** the founder's two follow-ups were each one sentence and each dissolved a problem I'd have solved the hard way. "Make it the width of the pill" turns the horizontal scale into exactly 1, so the squashing I was about to counter-scale away simply stops existing. "Make it part of the pill" turns a gap I'd tuned to 10px into an overlap of −26. Both times the simplification came from him.
**The rAF one:** four verification attempts in a row reported a sheet that had not opened, because `document.hidden` is permanently true in this browser pane and `requestAnimationFrame` never fires. That is my environment — but `Sheet.tsx` releases its enter animation on a bare rAF too, so a sheet mounting while the app backgrounds leaves the scrim up, the body scroll-locked, and no panel. Filed.
...
**Product/creator learning:** None of the three switches had `role="switch"` or `aria-checked`. A VoiceOver user heard "Toggle run notifications, button" and could not tell whether notifications were on — on the control that decides whether the product may speak to them at all. Making `checked` a required prop caught a live latent bug on its first compile: the auto-adjust prop was `?: boolean` while the state defaults to `true`, so an undefined would have announced "off".
**AI-building learning:** I took a brief to the board saying four switches. One was `SmokeToggle` with **zero call sites** — a feature removed from all UI in Phase 1. The settled-ground scan caught it, not me, and I would otherwise have spent a ruling on the palette of unreachable code. Measure the population before the meeting, every time.
**The honest bit:** Three separate population bugs in one day, all mine, all the same shape: a correct check pointed at an incomplete set. `sheetClose`'s hand-written consumer list never looked at the file rendering four of the nine sheets. The 44px floor arm measures only controls already on the system. And the geometry harness's `overlayTargets` hardcoded `btn|icon-btn` prefixes, so the switch's 44px hit area was invisible — adding switches without fixing that would have reported a correct control as broken. Then I hit the `[...matchAll]` tsconfig trap and the "vitest does not typecheck" trap in one file, both written down in this repo.
**Hook material:** The founder said "run notification toggles look mis shaped." Measured: untouched switch 26px, both converted 47px. Every test green.
**Postable?:** yes
...
**Dev learning:** the digest asked me to fix a 41% week-4 jump on an injured runner. Week 4 goes 24 → 17 → 24 km — it is a post-deload bounceback returning to the *exact* prior volume, net 0% across the deload, and the long run contributed 2 km of the 7. The +41% is measured against the deload trough, which is the wrong denominator, and §2 Amendment 3 makes that return explicitly legal for injury runners. **Three of the four items in the digest were like this.** The audit that "had not run since 12:55" has run nine consecutive days with no gaps; it just lands ~5 hours after its 07:45 cron because GitHub Actions schedules are best-effort, so nobody recognised the run. The two plans flagged with "9 and 6 foundation-week violations" contain **zero foundation weeks**.
**Product/creator learning:** that last one is the one worth keeping. `foundation_week_violations` is computed as `errors.filter(v => (v.week ?? 1) <= 0)`. Foundation weeks carry *negative* week numbers. But `week: 0` is this codebase's convention for "plan-level, no specific week" — 60 emit sites, documented in a comment on the first one. So the field counts plan-level violations and calls them foundation weeks. Across the live fleet it reports 50; one is a foundation week and 49 are not. **A field whose name states a cohort and whose arithmetic counts a different one will mislead every single reader, and it had already misled the digest, the board item and me for the first twenty minutes.**
**AI-building learning:** the real finding came from checking the premise I was asked to accept. Verifying that week 4 was fine meant reading which guard *should* have caught it — and finding that for this runner, none runs. `INV-PLAN-DELIVERED-RAMP` is gated `if (healthy)`. `INV-PLAN-INJURY-CAP-DELIVERED` matches only `knee` or `shin_splints`. Achilles, Back, Hip, Shin splints and Plantar fasciitis fall between them. Same input, injury value varied: the healthy runner gets four delivered-volume warnings at +23%; the Back and Plantar-fasciitis versions reach **+36% in silence**. §94 exists *because* healthy runners had no delivered check — and its gate created the mirror hole for the cohort with more reason to be guarded. **Declaring an injury currently removes a check that declaring nothing would have given you.**
**The honest bit:** `'Shin splints'` does not match `'shin_splints'`. Space versus underscore. The engine's matcher was fixed for exactly this on 2026-09-16; the invariant's was not, and the invariant's comment still says *"Predicate matches the engine's injury-cap gate exactly."* It has been false for nine days. `invariant:liveness` cannot see it, because both invariants are **proven wakeable** — via `knee`. **Liveness proves a rule can fire. It cannot prove it fires for the cohort it names, and we have no harness for that.**
**Also found:** `v_trial_conversion` — the view the 1 January 5% gate reads — counts the founder's own hand-seeded admin `stripe` row as a conversion. It reports 3.2%; the honest figure is 0%. GTM-CHARITY-05's migration comment *named this exact failure* five days ago and then fixed only `admin_user_tiers`. At 30 users, one admin row is 3.2 points of a 5-point threshold.
**Hook material:** I was asked to cap a 41% jump that turned out to be correct, and found instead that five of the six injuries our own wizard offers get no volume check at all. The one that does is the one the digest did not mention.
**Postable?:** yes
