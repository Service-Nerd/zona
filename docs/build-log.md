...
## 2026-09-18 — ONBOARD-EXIT-01 · The board told the founder no, and it was the right call
**Shipped:** A sign-out on every onboarding screen (there was none on three of four), and the optional first-name field on sign-up now says it is optional.
**Product learning:** The ask was "drop the first-name box, make it slicker". One look at the code showed the field was **already optional** — no validation, and the handler skips it cleanly. It just rendered as the first of three identical boxes above two that genuinely are required, with nothing to say so. Nobody skips a field they believe is mandatory, so the optionality existed only for whoever read the source. The friction was never the field, it was the disguise. The SLT declined the deletion unanimously and took the label instead: same slickness, and we keep the name that six prompt builders, the trial emails and the avatar all use. **Deleting it would also have part-reverted a fix shipped the previous day** — which is its own signal that the first decision was not finished being thought about.
**Dev learning:** Building the sign-out found a drift I wasn't looking for. ConnectRunsScreen already had this button, hand-rolled, and it had quietly diverged from the Me-screen version: no `clearWidgetState()`. A user at that screen **has** a plan, so signing out left the previous account's race countdown on the home-screen widget for whoever signed in next. Two copies, one wrong — the same duplicate-ownership class that has cost this repo a three-copy tier ladder and a five-copy deload cadence. I made it one component instead of a fourth copy, which is the only reason the drift got fixed at all.
**The honest bit:** I first shipped this saying the three onboarding screens were unseen — auth-gated, no fixture page, code and tests only. That is a true sentence and it is not an acceptable place to stop, so I built the fixture page (`/onboarding-preview`, the same dev-only pattern as `/me-preview`) and looked. **It found two things in about a minute.** On Connect Runs the new link landed directly under that screen's own "Not now →" skip, and at phone width the two are a near-identical pair of muted centred links: one skips a step, the other ends your session, on the single onboarding screen where you already have a plan to lose. And the `disabled` state changed only the CSS cursor, so a link that cannot be pressed looked exactly like one that can. Both are now fixed. **Neither was reachable by any test I could have written** — they are facts about two elements sitting next to each other, which only exist once something renders. The lesson is not "write a fixture page"; it is that "behind auth" is a description of the problem, not a reason to stop. **And then I was challenged a third time, and that was right too.** Everything above verifies that the code compiles and that the pixels are correct. None of it verifies that pressing the button does anything: the "proof" that the sequence ran in the right order was a test comparing two string positions in the source file. So I wrote tests that execute it, and then tried to break them — swap the order, drop the `finally`, drop the `await` — and all three go red. The `finally` turned out to be the load-bearing part: without it a failed `signOut` leaves the runner stranded on the exact screen this feature exists to escape. Then I clicked it for real in a browser and watched it land on the login page. **Three passes, three different kinds of evidence, and only the third one was about behaviour.** Compiling, rendering and working are three claims, and I had been reporting the first two as the third.
