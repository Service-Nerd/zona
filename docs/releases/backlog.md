...
**State at END of 2026-09-23 (last ship `a9ed2a7`, `PACE-UNITS-STEPS-01`) — PUSHED.** 3,260 tests · 366 files. 🥇 **I FIXED THE PACE IN THE MORNING AND THE FOUNDER STILL READ KM IN THE EVENING.** `PACE-UNITS-01` reached the pace TILE; **the step rows under it were never in scope** — `targetClause` returned `step.pace` VERBATIM while `BuildStepOpts` carried `formatDist` for the amount and **nothing for the pace beside it**. **36 of 38 step rows** and **2 of 2 race-pace lines** said `/km` → **0 and 0**. A row read **`~0.1mi · 5:30–6:00 /km`**: **two units on one line is worse than uniformly wrong.** ⚠️ **THE TRAP:** `buildRow` hands the SAME field to `paceMeanSecPerKm`, which is sec/**km** by definition — converting the field, not the display, would have silently corrupted every `~X mi` amount. 🥇 **THE GUARD HAD ALREADY WALKED THESE EXACT ROWS THAT MORNING**, for the raw-minutes rule, pinned at `units: 'km'` — right rows, different question. 🔴 **MY OWN NEW GATE WAS HOLLOW**: `/\bmi\b/` can never match `0.1mi` (no boundary between digit and letter); **caught only by falsifying, sixth boundary miss this week.** 🔴 **`npm run verify` HAD BEEN EXITING 1 ON MAIN ALL DAY** — 4 unbaselined slow tests from my own `REFUSAL-COHERENCE-01`, invisible because every test PASSES and the run prints the count four lines above the failure (`VERIFY-SLOW-BASELINE-01`). 🔻 **Filed, not fixed: `STEP-SUBUNIT-ZERO-01`** — a 30s recovery step reads `~0mi`; pre-existing, present in the before-probe. Earlier: `MODIFY-SHEET-01` (**"I just don't like it" is a symptom; measuring it cost me 2 of my own 4 findings** — 4+3 day grid, **race date DEFERRED needs a device**, **control vocabulary UNRESOLVED**), `PREVIEW-STRIP-01` (**two bar charts, only one meant anything**), `DATE-OWNER-01` (**asked for a preference, built the opposite**), `APP-SPACE-01` + `ACTION-ROW-01` (**we fixed the website and forgot the product**), `LONGRUNSHORT-RUNWALK-KEY-01`, `MARA-LR-SHAPE-SEAM-01`/§117 Am.4, `RUNWALK-VISIBLE-01`, `UNITS-DURATION-01`, `UNITS-PROSE-01` (CLOSED), `UNITS-SUBUNIT-01`, `PACE-UNITS-01`, `COMPLETION-TOMBSTONE-01`, `COACH-MEASURE-PROTOCOL-01`, `ZERO-REJECTION-SERVED-01`, `COHORT-SERVED-01`, `ONRAMP-STEP-UNITS-01`, `REFUSAL-COHERENCE-01`, `REFUSAL-FINISH-ROUTE-01`, `COACH-REVIEW-2026-09-23`, `RUBRIC-STALE-BAR-01`, `DB-USER-PURGE-01`.
