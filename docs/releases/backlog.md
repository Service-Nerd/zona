...
- 🔄 **App Store Connect setup** — partial (2026-05-08). Done: app record created (Bundle ID `app.zonna.ios`), subscription group `Zonna Premium`, monthly product `zonna_premium_monthly` (£7.99), annual product `zonna_premium_annual` (£59.99), 14-day free trial configured on both. **Copy drafted 2026-05-21** — description (~2050 chars), keywords (96/100), promotional text, per-product Review Information boilerplate all in `docs/releases/app-store-copy.md` ready to paste into ASC. Outstanding: capture 10 screenshots (5 shots × 2 device sizes per spec) + create Apple reviewer demo account (trial-expired, populated, `is_admin = false`) + paste copy + complete Privacy Details questionnaire (answers in `pre-submission-audit.md`).
...
- 🔲 **Full journey test** — agent-browser end-to-end: create account → onboarding → plan on screen → log session → post-log reflect → simulate trial end → attempt paid feature → upgrade prompt. **Script drafted 2026-05-21** as `docs/releases/journey-test.md` (10 steps with assertions + setup + cleanup + failure-mode table). Execution pending — run once per TestFlight cycle.
- 🔲 **App Store assets** — screenshots (all required device sizes), preview video (optional), keywords, App Store description copy. **Copy + spec drafted 2026-05-21** in `docs/releases/app-store-copy.md` (description, keywords, promotional text, 5-shot screenshot narrative with caption copy, per-product Review Information). Outstanding: actual screenshot capture in the simulator at iPhone 6.7" + 6.5" frames (data seeding instructions in the spec).
...
- 🔲 **HOOK-01 — Daily morning training-day push** *(scoped 2026-05-23)* — Fogg-prompt fix for the biggest behavioural gap in the product. 06:30 local push on every training day, content sourced from `getSessionVoiceLine`. Extracts voice lines from inline `DashboardClient.tsx` into testable `lib/coaching/voiceLines.ts`. New `daily_push_enabled` setting (default true for paid/trial). Strava-independent. Tier: PAID. Effort: M.
- 🔲 **VOICE-PATCH-01 — Brand voice patch (14 surface fixes)** *(scoped 2026-05-23)* — single PR applying all 14 voice slips from the review §5: login subhead, wizard step 1, Strava settings row, push row, `BRAND.push.runAnalysis`, skip-injury copy, strength completion km-70 line conditional, UpgradeScreen continue-free CTA, LockedCoachingPreview, signup success toast. Re-audit `brand-copy-alignment.md`. Strava-independent. Tier: FREE. Effort: XS.
- 🔲 **HOOK-02 — Day-3-of-trial "Kit noticed" push** *(scoped 2026-05-23)* — Cialdini-reciprocity nudge during trial. Fires once between trial day 3–5 when 2+ `run_analysis` rows exist. Body sourced from latest analysis `feedback_text`. New `trial_insight_push_sent_at` column. Strava-independent (reads source-agnostic `run_analysis`). Tier: PAID (trial-window). Effort: S.
- 🔲 **KIT-TASTE-01 — One free Kit insight per week, sourced from manual RPE** *(scoped 2026-05-23)* — partial-release conversion lever. Free user with ≥2 RPE logs in last 7 days gets one Haiku-generated insight per week, cached in new `free_insights` table. Replaces fully-dimmed `CoachTeaser`. CoachByline + AIMark. Risk-gated against overload signals via `reframeRiskGate.ts`. Strava-independent by design. Tier: FREE (Haiku-backed). Effort: M.
- 🔲 **SHARE-01 — Shareable weekly zone-discipline card** *(scoped 2026-05-23; tenure caption added 2026-05-23)* — `next/og`-rendered PNG for the weekly report (1080×1920 + 1200×630). Shares via `@capacitor/share`. Strava-independent — sources `% in zone` from source-agnostic `run_analysis.zones`. Includes tenure caption *"Holding the zone since {Mmm YYYY}"* sourced from `auth.users.created_at` (screenshot-only — explicitly *not* surfaced inside the app, and never on Me or the founder note). Suppressed for accounts created this calendar month. No `member`/`anniversary`/milestone language anywhere. Tier: PAID (paid users share; free users see upsell). Effort: M.
- 🔲 **CONNECT-01 — Connect-Your-Runs ceremonial onboarding screen** *(scoped 2026-05-23)* — Sutherland signalling. New screen inserted after `OrientationScreen` saying *"Kit needs your runs to do anything useful"*. Day one shows Apple Health only. Adds Strava as a second equal-weight CTA when approved — no redesign needed. New `connect_runs_seen` column. Tier: FREE. Effort: S.
- 🔲 **COMPLETE-01 — End-of-session screenshot-ready zone card** *(scoped 2026-05-23)* — Kahneman peak-end. New `<SessionCompleteCard />` shown after RPE entry. Renders zone bar when `run_analysis` exists, RPE + fatigue tag + completion copy when not. `BRAND.voiceAnchor` caption. Uses `@capacitor/share` (depends on SHARE-01). Strava-independent. Tier: FREE structure / PAID detail. Effort: M.
- 🔲 **FOUNDER-01 — Founder note page under Me** *(scoped 2026-05-23)* — Cialdini authority + liking + unity. Single ~300-word page in Zonna voice; no photo at v1. Tagged from Me row *"Why Zonna exists"*. Hosts `BRAND.brandStatement` at the bottom (paired with DOCTRINE-01). Strava-independent. Tier: FREE. Effort: S.
...
- 🔲 **STOP-01 — Cut tagline repetition** *(scoped 2026-05-23)* — pay off the brand asset by not over-using it. Tagline currently on splash + login + orientation. Cut from splash and orientation; keep on login. Splash + orientation use `BRAND.voiceAnchor` only. Update `brand-copy-alignment.md` §2. Strava-independent. Tier: FREE. Effort: XS.
