...
- 🟡 **SWEEP-VISIBLE-01 — mostly closed 2026-09-03 (`0c2081c`).** `INV-PLAN-MAX-WEEKDAY-MINS` 238 → **0** and `INV-PLAN-RACE-SPECIFIC-EXPOSURE-RATIO` 155 → **0**. `INV-PLAN-MIN-SESSION-SIZE` 2,061 → **924** — the remainder are low-volume main weeks whose easy runs fall under the §9 floor (§52b territory, unrelated to the weekday cap). **Verify still open:** `npm run verify:sweep` and read the "Known-open violations" block. Use `SWEEP_EXPLAIN=INV-PLAN-MIN-SESSION-SIZE npm run verify:sweep` for a fully reproducible failing input.
