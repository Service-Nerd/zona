...
**State at END of 2026-09-23 (last ship `d27bba0`, `UNITS-SUBUNIT-01`) — PUSHED.** 3,198 tests · 358 files. 🔴 **THE CARD TOLD 39.7% OF MILE RUNNERS THEY COVERED ZERO.** A 0.71 km cool-down is **0.44 mi**; `apportionRoundedDistance` floored it and printed **`~0mi`**. Measured through `resolveDisplayFigures`: **19,275 sessions (39.7%) in miles AND 1,805 (3.7%) in km** — cool-down in every mile case, real distances 0.25–1.27 km. A part apportioning to zero now shows its **duration** (2–9 min; **zero** cases of both-zero). ⚠️ **THE SUM SURVIVES BY CONSTRUCTION** — a part apportioned to 0 contributes 0, so SESSION-RECONCILE-01 cannot break; delta is **exactly** the 21,080 previously-zero parts, `mainEasy`/`racePace`/`mainSet` moved **zero** times. 🥇 **THE SAME-UNIT RULE LIVED ONLY IN A TEST, NEVER IN `ui-patterns.md`** — whose own shipped time-trial diagram renders `WARM-UP 10 min` beside a distance main set — **and the one shape that disproved it had been CARVED OUT of the assertion rather than examined. The carve-out was the tell** (`--surface-moss-wash` shape: rule and pattern in different documents, never meeting). Replaced with a **STRONGER** check: a minutes part must have apportioned to **exactly 0**, so the fallback cannot hide real ground. **KILLED:** whole-card-to-minutes (disables the toggle on ~40% of mile sessions) and `<1mi`. 🔴 **THE FILING RCA WAS WRONG TWICE** — *"never happens in km"* (it does) and it blamed `formatDistance`, which fires **0** times on session distances. ⚠️ **My instrument returned a clean ZERO first** — I omitted the pinned-date args, so all 902 plans refused; **an empty measurement looks exactly like a healthy one.** Earlier today: `PACE-UNITS-01`, `COMPLETION-TOMBSTONE-01`, `COACH-MEASURE-PROTOCOL-01`, `ZERO-REJECTION-SERVED-01`, `COHORT-SERVED-01`, `ONRAMP-STEP-UNITS-01`, `REFUSAL-COHERENCE-01`, `REFUSAL-FINISH-ROUTE-01`, `COACH-REVIEW-2026-09-23`, `RUBRIC-STALE-BAR-01`, `DB-USER-PURGE-01`.
