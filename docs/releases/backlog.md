...
**State at END of 2026-09-21 (last ship `d4de3cd`):** tree clean · `verify` exit 0 · **2,789 tests / 318 files** · **131 invariants** (code=doc, 0 orphans) · liveness **128/131 woken, 3 accounted, 0 unclassified** · `audit-docs.sh` ALL CLEAN. 🔴 **`DOC-AUDIT-WINDOW-01` — the ship-record check had TWO holes and both were found by hand, not by it.** Its window was `--since today`, so at 00:00 the list **emptied** and 2026-09-20's 39 scopes stopped being covered (one, `§117 Am.2`, had a registry row and no build-log entry — now backfilled). **Third date-scoped list in two days to expire and take its coverage with it.** And its scope pattern `[A-Z0-9-]+` **could never see 270 of 508 scopes (53%)**, `§117 Am.2` among them. Now a **commit marker that advances only on a clean run** (and **writes only when it changes**, so a read-only check stops dirtying the tree), plus a loose pattern. ⚠️ **Falsifying the fix found a bug in the fix** — `for id in $ids` word-split `§117 Am.2` into two ids. 🥇 **2026-09-20 was the big day: 39 ships.** All UX & flows buildable work is CLEAR (`P-15` · `P-04` · `P-09(a)(b)` · `P-10` · `P-12` · `P-14(a)` · `P-05(a)` · `P-08(a)` · `P-06(c)` · **`P-02`**), plus the engine, governance, commercial, security, legal and GTM categories. Full detail: [2026-09-20 day](project_2026_09_20_day.md). 🔻 **YOURS, and nothing else is blocking:** apply `20260920_charity_tier_and_partner_cohort.sql` · `OPS-VERCEL-PLAN-01` + `OPS-SUPABASE-PLAN-01` (P0) · `LEGAL-COUNSEL-01` · the `P-13(c)` illustration commission · the **second-typeface** decision that gates `P-06(b)` · **device verification: nothing built has ever run on one**, though `/refusal-preview`, `/zone-block-preview`, `/me-preview` and `/onboarding-preview` make it quick.
