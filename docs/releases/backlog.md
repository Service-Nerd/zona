...
### 🔻 `CHARITY-SEAT-MIGRATION-APPLY-01` — founder to run two SQL statements
**Board: ⚙️ NO BOARD.** Split from `CHARITY-CLAIM-RELEASE-01` (ruled and shipped 2026-09-24).
The code and docs are in; **the database is not changed yet.**

⚠️ **`20260924_charity_seat_stays_spent.sql` is deliberately NOT in
`.claude/state/applied-migrations.txt`** — the ledger records what is APPLIED, and it is not.
Add the filename only after step 1 succeeds, or the SessionStart hook warns every session.

**1. Apply the migration.** Paste `supabase/migrations/20260924_charity_seat_stays_spent.sql`
into the Supabase SQL editor. It replaces `purge_user_side_channels()` so account deletion no
longer clears `charity_codes.claimed_at`.
...
purged as cleanup, not real runners exercising deletion, so the ruling applies **going forward**
and the test batch stays usable. **Say so if you want them restored instead** — one statement
either way.

**Why I could not run either:** the Supabase CLI is not linked (OAuth needs an interactive
session) and the production data write was refused by the sandbox as a shared-resource change.
**Exposure meanwhile is near zero** — three codes, all on the TEST batch, and no real charity
batch has ever been created.
