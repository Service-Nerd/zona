...
- 🔲 **Strava as secondary source** *(post-launch)* — once HealthKit is primary, keep Strava OAuth + webhook + `strava_activities` writes alive but optional. Dedupe rule: if a HealthKit workout and a Strava activity match within ±5 min and ±5% distance, prefer the source with HR stream data; otherwise prefer HealthKit (always present on iOS). Apply for Strava API approval in parallel — not blocking v1. **The dedup rule is not yet implemented** — a 2026-05-23 HealthKit backfill created cross-source duplicates (the same run written to Health by both the Strava app and Garmin Connect, each a distinct `apple_health_uuid`, none deduped against the Strava-webhook row). A one-time manual cleanup ran 2026-05-30 (`scripts/cleanup-dupes.mjs`, 23 dupes removed for the founder account), but **the automated rule must land before R25 cohort cuts 2–3 ship** — those read `strava_activities` source-mixed and would otherwise count one run 2–3×, skewing similarity/trend output. Dedup on ingest, not on read.
...
- 🔲 **AI-DEPTH-02c — HealthKit per-km splits bucketing** *(scoped 2026-05-11; HR-drift shipped 2026-05-11 as AI-DEPTH-02a; Strava `splits_metric` path shipped 2026-05-24 as AI-DEPTH-02b)* — HealthKit doesn't natively give per-km splits. Server-side bucket the HR + distance samples on `raw_payload` into 1-km windows so HK-sourced runs get the same pace-fade signal that Strava runs now get via `splits_metric`. Output should be drop-in compatible with the existing `StravaSplitMetric` shape consumed by `lib/coaching/paceAnalysis.ts → computePaceFadeSummary()`. Effort: S (~1 day) — bucketing logic is straightforward; the consumer (paceAnalysis + the limiter classifier's muscular branch) is already in place. No schema change required. Tier: PAID.
