...
**State at END of 2026-09-20 (last ship `2b2fb33`):** tree clean · `verify` exit 0 · **2,789 tests / 318 files** · **131 invariants** · `audit-docs.sh` ALL CLEAN · **marathon 78.1%, whole product 92.3%**; every other distance clears the 90–95% bar. 🔴 **THE SCORE FELL TWICE TODAY BECAUSE THE RULER WAS WRONG, NOT THE ENGINE** — `measure:envelope` scored a REFUSAL as fit AND excluded everyone under 6 km/wk. **95.9% → 92.3%.** **Shipped: §116 on-ramp (dark) · §117 finish-goal run-walk (LIVE, peak 34) · §117 Am.2's adequacy bound · §118 BASE BUILDING** — the plan for everyone no race plan can serve. **Nobody who asks for a plan is told only no.** 🔴 **ZERO REJECTION IS NOT REACHABLE BY COACHING, confirmed twice.** Refusal 45.5% → 29.3% on the charity grid; the rest now get base building. 🔴 **TWELVE SHIPPED ITEMS WERE STILL MARKED OPEN** — one a P0 closed for two days — while `audit-docs.sh` read ALL CLEAN, because its backlog check only ever looked at TODAY's ship scopes (`BACKLOG-STALE-ALLTIME-01`). The widened all-time check found a twelfth my hand-audit missed. **Third time "an audit is only ever as wide as its list" has bitten.** 🟢 **`LOPSIDED-ORDER-01` closed: sweep errors 3 → 0**, baseline pinned at 0. ⚠️ **Its `verify:parity` came back IDENTICAL across 5,940 cases on a change the sweep proves altered 3 plans — a BLIND result, not a clean one.** ⚠️ **Two near-misses caught by governance, not by me:** an UNBOUNDED exemption that would have cleared 90% by admitting people to plans that do not work, and a 16-week bound chosen for LEGIBILITY that was setting a 4.17× physiological build limit. ⚠️ **Nothing has run on a device; the founder tests when this is done.** 🟢 **`S52-COMPOSITION-INV-01`: the converse of §64 now exists** — `INV-PLAN-WEEK-DELIVERS-DECLARED-DAYS`, warn, **0.9% (129/14,253)**. ⚠️ **My first cut checked `days_available` and fired on 45.1%** — that is §18's DESIGNED thinning, not a defect, and the board's own 17.2% figure was measuring it too. Re-scoped to the producer's own `max(3, …)` floor. **It observes the P0; it does not fix it** — the instrument stays deferred because the board's candidate breached the injury ceiling. New: `S80-VS-S90-PRIORITY-01`, the unruled collision every deferred instrument routes through. 🟢 **Coaching Board batch (4 items): 2 ruled, 2 WITHDRAWN under measurement.** §80 Am.2 — **§90's injury ceiling WINS**; the long run yields. ⚠️ **Both premises of the 09-19 sitting were FALSE** (§9 sizes and does not cap; §80 — not §24 — requires the 26 km, measured 208/210 min). ⚠️ **`MASTERS-COMPRESSED-BUILD-01` was a SAMPLING ARTEFACT** — 11pp on `measure:fitness`'s 3.4% slice, **0.8pp on the full 39,632-plan grid** (`FITNESS-BUCKET-SAMPLE-01` filed). `S111-LEVEL-INVERSION-01` closed as §79 working. **Engine backlog: nothing open.** 🟢 **Legal/privacy + security done.** `SEC-15` — **the filing said eleven of twelve and it was ten**: writing the coverage SWEEP (`everyAiRouteIsLimited.test.ts`) found a second unguarded Sonnet route, `/api/analyse-run`. ⚠️ **Deleting a guard left that test GREEN** (it matched the surviving `import`) — a route importing a guard and never calling it would have passed. Caught by falsifying. `CONSENT-DISCLOSURE-01` was **already shipped and stale in the backlog**; the gap was a GATE, now `healthConnectDisclosure.test.ts`. 🟢 **Commercial + engine governance CLEARED.** `PRICING-ROW-TRUTH-01` — **the first pricing row checked was FALSE** (*"No limit."* against a live 10/hr cap on every tier); the guard executes mechanical rows and pins reviewed ones to their text, and does **not** pretend to judge truth. `TT-PROJECTION-PROVENANCE-01` was **already built**. §24 Am.2 — the marathon long-run floor is **pace-conditional** (unreachable below ~6:38/km, by arithmetic). ⚠️ **`FITNESS-BUCKET-SAMPLE-01` was not a sample-SIZE defect: `targetedGrid` is entirely age 40, so masters and standard buckets were drawn from DIFFERENT GRIDS and were never comparable.** `COMPLIANCE-PROGRAMME` closed (`verify:coaching` HIGH 0). 🟡 **GTM & charity: all three done, ONE MIGRATION AWAITING YOUR APPLY.** `GTM-CHARITY-05` — `admin_user_tiers` restated `resolveTier`'s order and stopped one arm short; **measured live, 2 users read `trial` who hold an active grant** (500 in October, and `v_trial_conversion` would count every one as an unconverted trial). `GTM-CHARITY-06` — `v_partner_cohort`, six metrics, **both caveats shipped as COLUMN NAMES**. `GTM-CHARITY-07` — **fixed with NO migration**: `claimed_at` already survives deletion. 🔻 **YOURS: apply `supabase/migrations/20260920_charity_tier_and_partner_cohort.sql`** (validated read-only against prod; NOT yet in the applied-migrations ledger). 🟢 **`P-15` SHIPPED — and it is a REAL PLAN, not the stated route it scoped.** Its own premise was stale in our favour: §118 shipped the same day, `generateGetRunningPlan()` already returned a full validated Plan, and the refusal route was **throwing it away**. `RefusalView` **extracted** so `/refusal-preview` renders the real component in 5 states. ⚠️ **THREE FALSE GREENS while writing the gate** — the test first lived in `app/`, **which vitest does not collect, so it never ran**; then two source-slice anchors inspected the wrong region. 🔻 **Not walked on a device**, which is P-15's own acceptance criterion. 🟢 **`P-04` SHIPPED — the Plan screen has an intensity metric.** ⚠️ **The board had not sat and the SLT said it must; the measurement then REVERSED the ruling** — a 3-run minimum would have hidden the block on **57.1% of runner-weeks**, so the threshold gates the REGISTER not the visibility. ⚠️ **Two copy bugs caught pre-ship**, incl. a zero case reading *"Four of this week's runs stayed in the zone"* when four had DRIFTED. 🔻 **UX & flows: 9 left**; nothing device-walked. 🟡 **`P-09` (a)+(b) SHIPPED** — per-week pricing (**£1.15/wk annual, already beating the competitor's £1.54 with no price change**; the teardown's "~80p" implied a price we do not charge) and a trial timeline **whose every row is verified, not written**: the full-access guard enumerates the gates and is **falsified against the original `TIER-TRIAL-CONFIDENCE-01` defect**, and Day 11 is derived from `trialDays - 3`. 🔻 **(c) the exit offer deliberately NOT half-built** — needs a migration I cannot apply, and a screen no runner can reach is not an increment. 🟡 **`P-10` holes 1+2 closed** — dead cold-start code **deleted not fixed** (0 render sites; fixing it would put a non-existent bug in a build), and Coach stopped telling **web** runners to "Connect Apple Health" with no route to it (CTA **withheld, not relabelled**; deliberately silent on Strava, whose app is Inactive). ⚠️ **Deleting exposed doc rot and the new `Reference:` guard found a SECOND pre-existing break** (`SectionLabel`) in the doc `frontend-design` reads first. ⚠️ `useIsNative` owns the **render flag, not every platform check**. 🔻 **Hole 3: day one still never observed on a device.** 🟢 **`P-12` + `P-14(a)` SHIPPED.** The Me upsell **reads the gate-linked `/pricing` rows instead of retyping them** (falsified by retyping a real feature name), and states what you HAVE before what you lack. Review row added with a **derived** App Store URL, gated on it existing. 🔻 **`P-14(b)` deliberately not built** — Apple rate-limits to 3/year, so the trigger must be written down before it is coded. **UX & flows: 5 left** (`P-05`, `P-06`, `P-08`, `P-02`, `PLAN-NOTE-PLACEMENT-01`) plus `P-09(c)`. 🟢 **`P-08(a)` RESOLVED — the brief's proposal REJECTED on the code.** Moving redemption to the wizard's END would make `GTM-CHARITY-08` **strictly worse** (a comped runner cannot select the locked marathon to reach the step that unlocks it). ⚠️ **The filing was also stale** — the one redeem door is already on the DISTANCE step, beside the lock. Real fix: the gate sentence names **both** routes before the tap, as a wording change, not a second button. **UX & flows: 4 left** (`P-05`, `P-06`, `P-02`, `PLAN-NOTE-PLACEMENT-01`). 🟢 **`P-05` CLOSED, `P-08(a)` resolved, `PLAN-NOTE-PLACEMENT-01` reviewed.** P-05(a) ships plan length on every distance tile, **read from `PLAN_SIGNATURES`** (falsified by hardcoding one). ⚠️ **P-05(b) needed NO BUILD — its premise was stale**: a nameless runner already reads a sentence with no name, by design. **That is the second premise today to dissolve on reading the code.** 🔻 `PLAN-NOTE-PLACEMENT-01` left OPEN on purpose — both things that would settle it are unavailable — **but P-04 changed the screen it is about.** **UX & flows: 2 left (`P-06`, `P-02`).** 🟢 **`P-06(c)` SHIPPED** — the plan preview now leads with the block's shape (biggest week, total), **derived at render, never stored**. The adaptation promise is **tier-gated and verified first** (`dynamic_reshape_r20` is false for free) and **reuses the `/pricing` row's sentence** rather than making a second claim. ⚠️ **Chrome refused:** Dark Ground is marketing-scoped and the principles bar decorative dividers. 🔻 **(a) and (b) are FOUNDER-GATED, not unfinished** — an illustration commission, and a **second-typeface exception to the Inter-only rule**. **UX & flows: 1 left (`P-02`).** 🟡 **`P-02` SCOPED (its filing requires the board BEFORE scoping); build deliberately not started.** ⚠️ **Board VETOED a runner-facing intensity ratio** — 80/20 is a SESSION-COUNT rule, so at 4 days 80/20 vs 90/10 is **0.8 vs 0.4 sessions and does not quantise**; `hard_session_relationship` already expresses it under §110. 🟢 **Its named failure mode is already handled**: superseding is gated on `isRaceIdentityChange`, so editing days/cap/injuries **preserves** completions and only a race-date change supersedes. ⚠️ **I built 7 of today's 8 UX items WITHOUT loading `frontend-design`** — rules applied from `ui-patterns.md` directly, but the skill was only loaded for P-15 and P-02. **Noted, not excused; P-12's card and P-06's panel are the two worth a design look on device.** 🟢 **`P-02` BUILT AND SHIPPED — the last buildable UX item.** A runner can change days, cap, long-run day, injuries, hard sessions, terrain or race date **without re-running the wizard**. ⚠️ **Its named failure mode was already solved:** `editsResetLoggedWeeks` **asks the same `isRaceIdentityChange` that `savePlanForUser` uses**, so the warning and the behaviour cannot disagree. ⚠️ **Gated on `meta.generator_input` — 45% of live plans (10/22) get no entry point** rather than a regeneration from guessed answers. Board **vetoed the intensity ratio**. `verify:parity` IDENTICAL 5,940. **UX & flows: buildable work is CLEAR.** ⚠️ **AND I PUSHED P-02 WITH A GUARD RED.** `PREF-SWEEP-01` caught `${n} min` welded in the weekday-cap control; the failure was in a vitest run I did not read before committing. **Not a style violation** — ADR-015 locks ≥60 to hours, so it would have shown "60 min"/"90 min" against the rest of the product's "1h"/"1h 30". Fixed in `9b2bd13`; suite 2,789 green. 🟢 **DESIGN-REVIEW PASS over all 8 of today's UX items, via `frontend-design`.** Two real findings, both fixed: **(1) eyebrow tracking — the scale says `0.08em` and the codebase agrees 64:17**, but four of today's components inherited the CoachNoteBlock minority `0.14em` by copying their nearest neighbour. **Copy the scale, not the adjacent component.** **(2) six components shipped and NONE was documented** — the same drift that left `ui-patterns.md` pointing at a deleted `RestraintCard`. Added as pattern **37(a–f)**, including a **declared** new 22px three-up metric role (three metrics cannot be 44px at 375px). ⚠️ **An existing guard caught me mid-fix** — `uiPatternsIntegrity` rejected my `### 30` as a duplicate.
