...
**State at END of 2026-09-25 (last ship `b22cd4a8`, `SWITCH-PRIMITIVE-01`). 3,562 tests / 401 files. Docs ALL CLEAN.** 🔴 **THE DAY'S PATTERN IS POPULATION, NOT PREDICATE — three separate gates read clean while pointed at incomplete sets, all mine.** (1) `sheetClose`'s `CONSUMERS` was four hand-typed paths chosen because their FILENAMES say "Sheet", so it never looked at `DashboardClient`, **which renders four of the app's nine sheets** — the founder found the "This week's load" Close bar by tapping it. **The predicate was never wrong; it was never pointed at the file.** (2) The 44px floor arm measures only controls **already on the system** — i.e. not the population that violates it; filter dropped, **18 hand-rolled controls under 44px, smallest 18px** (`TAP-TARGET-FLOOR-01`). (3) The geometry baseline was keyed `file:line`, so **inserting ONE LINE re-keyed every control below it and the gate printed `moved: 0`** — not "nothing moved", **no longer looking**; falsified at **145 of 145 orphaned while the move arm stayed green**. 🥇 **THE CHECK WRITTEN TO FIND PRIMARY CTAs WAS BLIND TO PRIMARY CTAs** — it anchored its quote after the colon, and **a real CTA expresses a disabled state, so its colours are TERNARIES**. The founder named the Save button; I never checked it; widened → **nine more live CTAs at 3.68:1**, incl. the **wizard's Next**. 🟢 **`SWITCH-PRIMITIVE-01` — and the finding is not the pixel.** `ui-patterns.md` described a 44×26 toggle in four bullets and **nothing implemented it**, while a thousand lines below § Input Primitives said *"never build a one-off toggle inline"*. **The document forbade the only available method.** Collins: *"it contradicted itself and we called it drift."* **New rule: a section that names a control must name the component that implements it, or it is a sketch.** ⚠️ **My brief to the board said four switches — `SmokeToggle` has ZERO call sites**; the settled-ground scan caught it, not me. ⛔ **Silvanto NAMED a regression and declined to veto** — first time the seat has done that. 🥇 **`checked` as a required prop caught a live latent bug on first compile** (`?: boolean` against a `true` default → would have announced "off"). 📐 **157 controls · geometry moved 0 · under floor 0 · build exit 0.** ⚠️ **Nothing verified on a device, and this governs the control that triggers the real APNs prompt.**
