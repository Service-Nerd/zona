...
- 🔲 **AI-DEPTH-02 — HealthKit per-km splits + HR-over-time analysis** *(scoped 2026-05-11)* — biggest data-side win available. HealthKit's `HKWorkout` already gives per-sample HR with timestamps (`raw_payload.hrSamples` on `strava_activities` rows captured by `lib/health/adapter.ts`); we just don't analyse it. Build a `lib/coaching/streamAnalysis.ts` that computes HR drift slope (first third vs last third of run), late-fade pace ratio, and per-km HR profile from the raw samples already stored. Feed first/last/worst km plus drift metric into `buildSessionFeedbackPrompt` so feedback can be specific ("HR drifted 14 bpm in the back third — the fade started around 35km"). For Strava-sourced activities, also pull `splits_metric` at link time when API access lands (`app/api/webhooks/strava/route.ts` enrichment step). Tier: PAID. Effort: M (~1 week). No schema change — `raw_payload` already has the streams; the adapter discards them once aggregated. Optionally persist a derived `stream_summary` JSONB if recomputing on every read becomes a cost.
...
- 🔲 **AI-DEPTH-04 — Conversation memory across weekly reports** *(scoped 2026-05-11)* — when the weekly report fires, inject the previous week's `headline` + `body` into the prompt so the model can reference progress or repeat-issues. Pattern: *"Coach last week: 'Easy runs weren't easy.' This week: zone discipline rose from 52 → 78 — kept under control."* `app/api/weekly-report/route.ts` already has `weekly_reports` table read access; just SELECT the prior row keyed `(user_id, week_n - 1)` and pass headline+body into `buildWeeklyReportPrompt` as a new optional `previousReport` block. Add an explicit prompt instruction that the model may reference the previous report at most once and only when the data tracks against it. Tier: PAID. Effort: S (~3 days). Closes the "every report fires standalone, no continuity" gap.
...
- 🔲 **AI-DEPTH-07 — Plan schema extensions (prerequisite for AI-DEPTH-08)** *(scoped 2026-05-11)* — additive fields on `Session`/`Week`/`PlanMeta`: `key_session: boolean` (flag sessions that are pivotal — long runs, race-pace tempos, planned tune-ups), `run_walk_strategy: string` (e.g. "9:1 from km 10"), `fueling_protocol: string` (e.g. "gel every 25 min from km 10"), `result_embedded: RaceResult | null` on `Week` (race-day result captured into the plan for downstream consumers to reference). Pure schema work — no engine changes — but every plan consumer needs an audit pass. Tier: FREE (infra). Effort: S (~3 days). Standalone value is low; unlocks AI-DEPTH-08.
