...
- 🔄 **Sign in with Apple** — Apple §5.1.1d, mandatory because Google OAuth is present. Engineering shipped 2026-05-09:
  - ✅ `@capacitor-community/apple-sign-in@7.1.0` installed; `npx cap sync ios` ran
  - ✅ `com.apple.developer.applesignin = ["Default"]` added to `ios/App/App/App.entitlements`
  - ✅ Login page (`app/auth/login/page.tsx`) Apple button above Google (HIG: equivalent prominence). Native path: nonce → `SignInWithApple.authorize` → `supabase.auth.signInWithIdToken({ provider:'apple', token, nonce })` → `supabase.auth.updateUser({ data: { full_name } })` on first-auth name capture. Web path: `signInWithOAuth({ provider: 'apple' })`. Note: the `signInWithIdToken` API doesn't take `options.data` (only `nonce`/`captchaToken`), so the name pass-through goes via a follow-up `updateUser` call rather than the inline form the original backlog text suggested.
