...
- 🔄 **HealthKit primary data source** ⚠️ **App Store blocker — supersedes Strava as v1 data source.** All TS/SQL code shipped. Only gated work remains: native plugin install, Xcode capability, Info.plist usage description, on-device smoke test. Strava is still in Single Player Mode (1-athlete cap), API approval is opaque + slow, and we cannot run TestFlight beta on a single token. Apple Health needs no approval. Audit (2026-04-30) confirmed `lib/coaching/*` is source-agnostic — every consumer reads `strava_activities` rows as a canonical internal shape, not a Strava-shaped contract. HKWorkout maps 1:1; HR samples replace the Strava stream summary. Strava becomes an optional secondary import post-launch. **Status (2026-05-05):** data + intelligence layer fully shipped; only the Apple-side wiring + one wizard pre-fill remain. Sub-tasks:
  - 🔲 Install plugin (`@capacitor-community/health` or `capacitor-health` — pick during build; both expose HKWorkout + HKQuantitySample). Add `HealthKit` capability to Xcode App target. Add `NSHealthShareUsageDescription` to `ios/App/App/Info.plist` ("Vetra reads your runs to provide coaching feedback. Vetra never writes to Apple Health."). HealthKit capability enabled in Apple Developer portal 2026-05-08.
  - ✅ Migration `20260501_healthkit_primary.sql` — `source` + `apple_health_uuid` columns on `strava_activities` (full unique constraints, NULLs-distinct semantics — replaces original "partial unique indexes" plan because Supabase upsert can't infer them); `health_daily_samples` table; `user_settings.healthkit_connected_at`; `session_completions.apple_health_uuid`; `run_analysis` source-agnostic IDs; `plan_adjustments.trigger_type` accepts `readiness_signal`
  - ✅ `lib/health/adapter.ts` — pure HKWorkout + HR samples → `strava_activities` row mapper
  - ✅ `lib/health/clientSync.ts` — foreground sync on app open (note: file named `clientSync.ts`, not `sync.ts`)
  - ✅ `POST /api/health/ingest` + `POST /api/health/samples` — workout + daily sample ingest, mirrors Strava webhook enrichment pipeline
  - ✅ HealthKit connect/disconnect UI in MeScreen (DashboardClient.tsx ~line 5871) — calls `requestHealthKitAuth` + writes `healthkit_connected_at`
  - ✅ Feature gate renamed `strava_intelligence` → `activity_intelligence` in `lib/plan/featureGates.ts` and all call sites
  - ✅ `/privacy` page HealthKit disclosure section (page line 126–128) — read-only, never written, on-device-only, revoke from iOS Settings or Profile
  - ✅ **Pre-fill RHR + MaxHR** — wired into `HRZonesSection` (DashboardClient.tsx:6049 `AppleHealthPrefillButton`), gated on `Capacitor.isNativePlatform()`, calls `fetchAppleHealthHRSnapshot` from `lib/health/clientSync.ts`. Backlog originally said `BenchmarkUpdateScreen` (no HR fields) and the wizard `GeneratePlanScreen` (state but no UI input) — actual edit surface is MeScreen, where this lives.
  - ✅ VO2 max → race-times sanity check (`app/api/race-times/route.ts` `computeVO2CrossCheck`, ±10% divergence flag)
  - ✅ Readiness signal trigger (`lib/coaching/planAdjustment.ts` — `readinessSignal` input on `AdjustmentCheckInput`, RHR/HRV/sleep voting against 14-day baseline, fires pre-session on hard days)
  - 🔲 Smoke test: agent-browser end-to-end on simulator with seeded HealthKit data — gated on plugin install
...
- 🔄 **Sign in with Apple** — Apple §5.1.1d, mandatory because Google OAuth is present. Engineering shipped 2026-05-09:
  - ✅ `@capawesome/capacitor-apple-sign-in@0.1.0` installed; `npx cap sync ios` ran. Originally tried `@capacitor-community/apple-sign-in@7.1.0` but its `capacitor-swift-pm` constraint (`7.x`) conflicted with `@capgo/capacitor-health@8.x`'s constraint (`8.x`), causing Xcode SPM resolution failure. Swapped to Capawesome's Capacitor-8-native plugin (2026-05-09); APIs are slightly different but functionally equivalent (see login page).
  - ✅ `com.apple.developer.applesignin = ["Default"]` added to `ios/App/App/App.entitlements`
  - ✅ Login page (`app/auth/login/page.tsx`) Apple button above Google (HIG: equivalent prominence). Native path: nonce → `AppleSignIn.signIn({ scopes, nonce })` → `supabase.auth.signInWithIdToken({ provider:'apple', token, nonce })` → `supabase.auth.updateUser({ data: { full_name } })` on first-auth name capture. Web path: `signInWithOAuth({ provider: 'apple' })`. Note: the `signInWithIdToken` API doesn't take `options.data` (only `nonce`/`captchaToken`), so the name pass-through goes via a follow-up `updateUser` call rather than the inline form the original backlog text suggested.
  - ✅ `apple-logo.svg` asset added to `/public`
  - ✅ CLAUDE.md updated — `@capacitor-community/apple-sign-in` moved to "installed", name-handoff gotcha documented at architecture level
  - ✅ Apple Sign in capability enabled in Apple Developer portal 2026-05-08
  - ✅ **Xcode capability** — Sign in with Apple ticked in App target → Signing & Capabilities (2026-05-09). The entitlement key was already in `App.entitlements`; Xcode UI now tracks it so it survives clean rebuilds.
  - ✅ **Supabase provider config** (2026-05-09) — Apple provider enabled with `app.vetra.ios` in **Authorized Client IDs**. Native-only path: `signInWithIdToken` verifies the id_token directly against Apple's public keys, so no Services ID and no .p8 key are needed. Redirect URL `app.vetra.ios://auth-callback` was already in place from Google OAuth. Web Apple sign-in deferred — if ever wanted, add a Services ID (`app.vetra.signin`) + .p8 key later; doesn't affect the iOS path.
  - 🔲 **Test on a real device** — Apple sign-in works in the simulator if you're signed into iCloud, but the first-auth name capture is most realistic on a fresh device. Reset trick to re-test first-auth: iOS Settings → Apple ID → Sign in with Apple → Vetra → Stop using.
  - **Name-handoff gotcha (preserved at code level):** Apple returns `givenName` + `familyName` *only* on the very first authorization — never again. The login handler captures these and writes them via `updateUser({ data: { full_name } })`; the pre-fill in `DashboardClient.tsx:495–505` then reads `user_metadata.full_name` and writes first/last to `user_settings`. Don't remove that `updateUser` call — there is no way to recover the name from Apple after the first auth.
