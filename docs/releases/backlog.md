...
### 🧭 `CTA-FLAT-01` — every CTA button in the product reads flat
**Board: 🧭 DESIGN BOARD.** Founder observation, 2026-09-24: *"I do want it logged though that I
find all our cta buttons very flat visually."* **Logged, not built** — the founder asked for it to
be recorded, and the fix is a design decision, not a defect fix.

⚠️ **Scope is the WHOLE product, not the emails.** *"All our CTA buttons"* — app, marketing site
and email. Any answer has to hold on a surface that cannot run CSS (email) and one that can.

🔴 **Measured, and the measurement is sharper than the observation.**

| | |
|---|---|
| `--shadow-card` | used by **19 files** |
| `--shadow-lifted` | **0 consumers.** Declared, never used |
| `--shadow-device` | 1 (the marketing phone frame) |
| The only CTA class in `globals.css` | `.cta-pill`, which is **a background swap on hover and nothing else** |

**So the product elevates its CARDS and gives its BUTTONS nothing.** A moss fill on a white card,
with no lift, no border, no depth and a hover that only darkens — the button is visually *behind*
the card it sits on. That is the flatness, and it is a token-layer fact rather than a matter of
taste.

⚠️ **`--shadow-lifted` having zero consumers is the decorative-config class** (`--s-long`,
`run_walk_strategy`, `ZONE_DISCIPLINE_BANDS`). It was authored for exactly this job and nothing
ever used it.

**Constraints any proposal must respect:**
- **Email cannot use `box-shadow` reliably** — Outlook drops it. An email CTA needs depth from
  something that survives: border, contrast, size, or a second colour stop.
- `.cta-pill` already rides `--motion-ui`, which collapses under `prefers-reduced-motion`. A
  press/lift state inherits that for free.
- Silvanto's standing correction applies: **restraint is a hierarchy decision, not an absence of
  one** — and the no-chrome kill stands, so *"add a gradient"* needs an explicit overturn of that
  rule by name, not an exception.

**For the board:** does the primary CTA earn elevation, and if so which token — the unused
`--shadow-lifted`, or a new button-specific one? And what is the email-safe equivalent?
