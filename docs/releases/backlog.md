...
**State at END of 2026-09-23 (last ship `517d58f`, `COMPLETION-TOMBSTONE-01`) — PUSHED.** 3,150 tests · `review:cohort` unchanged, every band. 🔴 **LIVE DEFECT FIXED: a completion write landed on a SUPERSEDED row and inherited its tombstone.** The founder's skip-for-illness and manual-log *"would not save"* — **they saved perfectly, onto rows created 2026-04-14 on the previous plan.** `week_n` is **within-plan** (ADR-013); a new plan restarts at 1 and the upsert key `(user_id, week_n, session_day)` is **not plan-scoped**. 🥇 **`PLAN-WEEK-COLLISION-01` FIXED THE READ SIDE AND LEFT THE WRITE SIDE COLLIDING** — the *twin-transition* class already recorded against ADR-013. ⚠️ **AND IT MERGED TWO PLANS' DATA** — an illness-skip carrying **`rpe = 3` from April** into the fatigue triggers. **FIX: partial unique index `WHERE superseded_at IS NULL`** so a new plan **inserts beside its history**; write moved to an RPC (PostgREST cannot target a partial index). 🥇 **THE GATE FOUND FOUR MORE SITES I HAD MISSED**, all server-side — including **both** `healthkitConsolidate` mirrors, where my first edit hit the textually-identical `run_analysis` one. **Blast radius 150 rows / 1 user — a live LANDMINE, not an outage**; hits everyone who uses *Start a new plan*. **Repaired 2 rows.** Earlier today: `COACH-MEASURE-PROTOCOL-01`, `ZERO-REJECTION-SERVED-01` (marathon **89.8%**, still 0.2pp under target), `COHORT-SERVED-01`, `ONRAMP-STEP-UNITS-01`, `REFUSAL-COHERENCE-01`, `REFUSAL-FINISH-ROUTE-01`, `COACH-REVIEW-2026-09-23`, `RUBRIC-STALE-BAR-01`, `DB-USER-PURGE-01`.
