...
**State at END of 2026-09-20 (last ship `eec5272`):** tree clean · `verify` exit 0 · **2,613 tests / 297 files** · **129 invariants** · `audit-docs.sh` ALL CLEAN. 🔴 **THE SCORE MOVED THREE TIMES TODAY AND TWICE IT WAS THE RULER, NOT THE ENGINE.** `measure:envelope` scored a REFUSAL as fit, and its population excluded everyone under 6 km/wk. **Whole product 95.9% → 92.3%, marathon 90.1% → 78.1%.** Every other distance clears the 90–95% bar (5K/10K/50K/100K 100%, HM 96.2%). **The marathon is the only failure and 12.4 of its 11.9-point gap is REJECTION.** **Shipped: §116 base-build on-ramp** (dark — the remedy §111 named and §57 made impossible, flat from week 2 at any length) · **§117 finish-goal run-walk, LIVE at peak 34** (§111's door 13 → 9; nothing loosened, the door is downstream of the target) · **§117 Am.2's adequacy bound.** 🔴 **THE CLOSEST CALL: `LONG-RUN-SHORT` was exempted on §117 plans for a good reason and the exemption had NO BOUND** — 11–13 km long runs were scoring FIT, and dropping the on-ramp floor would have cleared 90% **by admitting people to plans that do not work.** Chair caught it; §117 now REFUSES rather than hand one over. ⚠️ **An invariant was not enough — in production `validatePlan` only LOGS.** 🔴 **ZERO REJECTION IS NOT REACHABLE BY COACHING, confirmed twice by independent routes.** 45.5% → 29.3% on the charity grid is the ceiling. **Founder has directed that the remainder get a GET-RUNNING plan** — product decision made, in progress. ⚠️ **Nothing has run on a device; the founder tests when this is done.** 🔻 **Yours:** `OPS-VERCEL-PLAN-01` + `OPS-SUPABASE-PLAN-01` (P0) · `LEGAL-COUNSEL-01` · `SIGNPOST-HARM-01` · the `/pricing` wording · the net-revenue conclusions · `P-04`'s zero-case words.
