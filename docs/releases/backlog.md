...
**State at END of 2026-09-20 (last ship `bbc40ef`):** ✅ tree clean. **2,485 tests / 283 files** · **124 invariants** · sweep **14,189 plans, no new violations** · `verify:parity` IDENTICAL · `audit-docs.sh` ALL CLEAN. 🥇 **FIT FOR PURPOSE 66.7% → 95.3%. EVERY DISTANCE AT OR ABOVE 90%:** 5K 100% · 10K 100% · HM 96.2% · marathon 90.1% · 50K 100% · 100K 100%. **Final board sitting: 24 of 27 plans proud to hand over, NO NEW ISSUES** — the first sitting of which that is true. 🔴 **WHY THE BOARD'S ANSWERS KEPT CHANGING — it was not the board.** The review round's `review.md` said *"REVIEW PENDING"* and nobody filled it, so every sitting was convened from the PLANS instead of the prior RULINGS. M3 was WITHDRAWN 09-19 and re-presented 09-20 as "the one real finding"; E5 was CLOSED 09-19 and re-presented as "would not hand over". **Fixed: `docs/canonical/coaching-rulings.md` is the standing register (read it BEFORE the plans), and `audit-docs.sh` now fails the build when a round's ruling is never written back** — it found a second round pending since 09-15, five days. 🟡 **Open, all filed with measurements:** `MARA-LR-LOWBASE-01` (P1 — 8 km/wk marathoners get a 16.5 km peak long run; admitted *because* the injury cap lowers their peak so §111's ratio passes) · `ULTRA-LR-ADEQUACY-01` · `S111-SUBFLOOR-VOLUME-01` · `RACE-KEY-TWO-OWNERS-01` · `GRID-EARLY-ONSET-01` · `RUBRIC-GAPS-01`. ⚠️ **Device verification founder-owned and OPEN — nothing has run on a device.**
