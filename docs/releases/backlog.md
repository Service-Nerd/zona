...
**State at END of 2026-09-23 (last ship `b9aa5a9`, `APP-SPACE-01` + `ACTION-ROW-01`) — PUSHED.** 3,239 tests · 363 files. 🔴 **WE FIXED THE WEBSITE AND FORGOT THE PRODUCT.** `SITE-WAVE-4` ruled `--space-1…7` and swept the SITE; a day later `var(--space-*)` appeared **17× in `components/marketing` and ZERO times in `app/dashboard` or `components/shared`** — **578 hand-typed gaps, 25 distinct values, 13 off-scale**. The site had **24/19** when ruled: **the app was in the state the site was in before the fix**, and Wave 4's OWN row had warned *"a token family nobody applies is the `surface=` failure repeated."* Swept **295 no-change + 277 shifted, none >4px**. ⚠️ **The tie breaks UPWARD — my first cut rounded DOWN and tightened 74 gaps on the day the complaint was crowding.** ⚠️ **Bottom clearance is NOT a gap** (tab-bar room, declared). 🔴 **The sweep could not have fixed the REPORTED case** — § 332: *a gap of zero is an ABSENT DECISION*; the tile declared **no top margin at all**. 🥇 **`ACTION-ROW-01`: A PATTERN THAT IS A LOCAL VARIABLE CANNOT TRAVEL** — the chevron was a local `const` inside the Me screen, 7 rows used it, the Plan screen could not reach it, so its tile shipped with **no affordance**. ⚠️ **3 of 6 findings SENT BACK for a measurement**: dates (**9 formats, 22 sites, no owner, one raw ISO — and NO preference exists to follow**, so SLT first), two charts on the preview, the modal's geometry. Earlier: `LONGRUNSHORT-RUNWALK-KEY-01`, `MARA-LR-SHAPE-SEAM-01`/§117 Am.4, `RUNWALK-VISIBLE-01`, `UNITS-DURATION-01`, `UNITS-PROSE-01` (CLOSED), `UNITS-SUBUNIT-01`, `PACE-UNITS-01`, `COMPLETION-TOMBSTONE-01`, `COACH-MEASURE-PROTOCOL-01`, `ZERO-REJECTION-SERVED-01`, `COHORT-SERVED-01`, `ONRAMP-STEP-UNITS-01`, `REFUSAL-COHERENCE-01`, `REFUSAL-FINISH-ROUTE-01`, `COACH-REVIEW-2026-09-23`, `RUBRIC-STALE-BAR-01`, `DB-USER-PURGE-01`.
