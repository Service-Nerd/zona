...
### ⚙️ `PLAN-STORED-SCHEMA-DRIFT-01` — nothing parses a STORED plan back through `PlanSchema`
**Board: ⚙️ NO BOARD.** Filed 2026-09-24, split out of `PLAN-LEGACY-ZONE-STRING-01` when that
shipped. **This is the general form, and it is worth more than the zone strings were.**

`PlanSchema` guards the GENERATE path. Nothing re-validates a plan already in the table, so a
row that drifts is read by components that take what they find. Measured against production
2026-09-24: **20 of 22 plans valid, 2 not**, and neither was reachable by any current producer.

- `29bf01c9` (created 2026-04-23) — 11 sessions carry a `type` outside the current enum. Pre-R23.
- `e916f15a` (created 2026-05-28) — `meta.handle` / `meta.charity` are `null` where the schema
  says `z.string()`. Every current producer writes `''` (`lib/plan.ts:19/30`,
  `ruleEngine.ts:8915/8919`), so no new plan can take this shape.

⚠️ **The two rows are the symptom, not the item.** Three separate live-data defects this week
(`PLAN-ZONE-VS-HRTARGET-01`, `PLAN-RESTING-HR-ZERO-01`, `PLAN-VO2MAX-BAND-01`) were all found by
someone happening to look, and all three were **invisible to every gate in the repo**, because
every gate runs on generation. **A validator that only runs on write cannot see the table rot.**

**The cheap version is an ops read, not read-time enforcement:** a script (or a `/api/ops/` route)
that parses every stored plan and reports the invalid ones, so drift is observable. Refusing to
RENDER an invalid stored plan is the wrong move — the runner would lose their plan over a field
they never see.
