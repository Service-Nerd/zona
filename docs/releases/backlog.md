...
**State at END of 2026-09-20 (last ship `507e85d`):** tree clean. **2,487 tests / 283 files** - **124 invariants** - sweep 14,189 plans no new violations - `audit-docs.sh` ALL CLEAN. **FIT FOR PURPOSE 66.7% -> 95.9%** (baselined), every distance at or above 90%: 5K 100% - 10K 100% - HM 96.2% - marathon 90.1% - 50K 100% - 100K 100%. **Final board sitting: 25 of 28 proud to hand over, NO NEW ISSUES.** **TO RE-RUN THE REVIEW COMPARABLY:** `coaching-review-round.ts` then `npm run measure:envelope` (diffs vs `envelopeBaseline.json`, fails on a move EITHER way), `npm run audit:plans`, `npm run verify`. Procedure + what is NOT comparable: `docs/canonical/coaching-rulings.md`. **Why the board seemed to change its mind:** the round's `review.md` said REVIEW PENDING and nobody filled it, so sittings were convened from the PLANS not the prior RULINGS. Fixed by the rulings register (read FIRST) + an audit gate. **M4 was never a refusal** - `warn_unacknowledged` is a CONFIRMATION PROMPT; the persona said `expectRefusal` for months. `M6` added for genuine block coverage. **Open, all filed with measurements:** `MARA-LR-LOWBASE-01` (P1) - `ULTRA-LR-ADEQUACY-01` - `S111-SUBFLOOR-VOLUME-01` - `RACE-KEY-TWO-OWNERS-01` - `GRID-EARLY-ONSET-01` - `RUBRIC-GAPS-01`. **Device verification founder-owned and OPEN - nothing has run on a device.**
