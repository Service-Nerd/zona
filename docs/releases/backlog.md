...
| ⛔ | ENGINE-03a — Cycle false positives (BLOCKED — see ENGINE-03 below) | S | FREE |
...
- 🔄 **DS-07 — Composite effort logging** *(Part A shipped; Part B open, ~1.5 days)* — two or more activities of the same type that together satisfy one planned session (e.g. hike + treadmill top-up = one planned long easy run). Distinct from Supplementary session slots (which is strength/cross-train alongside a run).

  **Part A — Edit logged distance — ✅ SHIPPED 2026-06-22 → feature-registry.** (distance-correction on a manual completion).
