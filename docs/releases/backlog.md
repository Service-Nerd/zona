...
- 🔄 **TestFlight beta** — build uploaded and processing 2026-05-15 (second upload after adding `NSHealthUpdateUsageDescription` to Info.plist — required by Apple when HealthKit entitlement is present even when app is read-only). **⚠️ A fresh archive is required before device smoke** — the 2026-05-15 build predates the LaunchScreen redesign + all late-May/June work (forms primitives, data-source doctrine / HealthKit-first UI, post-race reshape, push-permission onboarding, post-race goal-race detection fix). Re-archive in Xcode → upload → then: add yourself + the 1–2 current testers as internal testers, install on device, run the on-device journey legs.
