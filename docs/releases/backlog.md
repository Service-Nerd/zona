...
**State at END of 2026-09-23 (last ship `663bbba`, `PACE-UNITS-01`) — PUSHED.** 3,193 tests · 358 files. 🔴 **THE UNITS TOGGLE CONVERTED DISTANCE EVERYWHERE AND PACE NOWHERE.** `formatPace(353,'mi')` returned `9:28/mi` **correctly the whole time** and was simply **unreachable**: `ruleEngine.ts` welds `/km` onto `pace_target` as a **STRING at GENERATION time**, so by render there is no number left to convert — **674 of 888 stored sessions, 17 of 19 plans**. 🥇 **FIXING THE PRODUCER WOULD HAVE REPAIRED ZERO EXISTING PLANS**, so the fix goes at the READ (`convertPaceString` → hands the digits to `formatPace`, which still does the arithmetic). ⚠️ **ORDER IS LOAD-BEARING**: `easyPaceAsCeiling` sniffs the unit back OUT of the text, so convert-then-ceiling is correct and the reverse silently reintroduces the defect — **both** now asserted; all 6 render sites verified. 🥇 **THE GUARD HAD TWO INDEPENDENT BLIND SPOTS AND EITHER ALONE HID THIS**: the regex had no `/` (matched `${x}km`, not `${x} /km`), and `git ls-files 'lib/plan/**/*.ts'` returns **ZERO** because `lib/plan` is flat — **393 files** its own header claimed to cover were never opened. **I trusted the header instead of asking the check what it could SEE.** Widening both surfaced **87** more: 25 validator messages excluded by whole file, **62 filed as the CLOSED `UNITS-PROSE-01` register**. ⚠️ **MEASURED, AND IT CUTS BOTH WAYS: 19 stored plans carry ZERO prose km while 17 carry `/km` pace** — real, unrealised, and **NOT** evidence it cannot fire (§80's note named its cap 0 times in 5,264 plans). Tested against all **27 distinct production `pace_target` shapes**. Earlier today: `COMPLETION-TOMBSTONE-01`, `COACH-MEASURE-PROTOCOL-01`, `ZERO-REJECTION-SERVED-01`, `COHORT-SERVED-01`, `ONRAMP-STEP-UNITS-01`, `REFUSAL-COHERENCE-01`, `REFUSAL-FINISH-ROUTE-01`, `COACH-REVIEW-2026-09-23`, `RUBRIC-STALE-BAR-01`, `DB-USER-PURGE-01`.
