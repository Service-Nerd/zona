...
**State at END of 2026-09-23 (last ship `edddbcc`, `LONGRUNSHORT-RUNWALK-KEY-01`) — PUSHED.** 3,229 tests · 361 files. **Marathon 89.5%** (day net **89.8 → 89.5**, with **16.7pp of the low-base INJURED cohort newly served**). 🥇 **§117 Am.4: I SHIPPED A CHANGE THAT MADE THE HEADLINE WORSE, ON PURPOSE, AND PREDICTED IT FIRST** — §117 bundled a **lower peak** with a **run-walk instruction**; a §12-capped runner needs the second and **not** the first (their door is already open), so they paid twice and their long run went **17→13km**, below §117 Am.2's bound, and the gate **REFUSED** them. **Isolated by SWEEPING THE CONSTANT** (34→13 … 52→17). Refusals @8km/wk **85.8→69.1 (−16.7pp)**, `LONG-RUN-SHORT` **+16.6pp** — **matching one-for-one.** 🔴 **THEN I BROKE THE SCORING RULE WITH MY OWN FLAG SPLIT** and fixed it hours later (`run_walk_prescribed`, bounded at 17km): **+0.4pp, NO PLAN CHANGED**, declared a CORRECTION. ⚠️ **FOUR INSTRUMENT FAULTS in one thread**, worst: **I told the board the headline wouldn't move, having checked `cohortGrid` (no injury) when the rate comes from `useCaseEnvelope` (50% injured)** — confidently stated, unverified, wrong. 🥇 **Every fault was computing something BESIDE the owner instead of ASKING the owner.** ⚠️ **The envelope gate has a 0.6pp TOLERANCE** — a 0.4pp move passes silently; *green* means *small*, not *unchanged*. Earlier: `RUNWALK-VISIBLE-01` (**a prescription written and rendered NOWHERE, 0 commits ever**), `UNITS-DURATION-01`, `UNITS-PROSE-01` (4 phases, CLOSED), `UNITS-SUBUNIT-01`, `PACE-UNITS-01`, `COMPLETION-TOMBSTONE-01`, `COACH-MEASURE-PROTOCOL-01`, `ZERO-REJECTION-SERVED-01`, `COHORT-SERVED-01`, `ONRAMP-STEP-UNITS-01`, `REFUSAL-COHERENCE-01`, `REFUSAL-FINISH-ROUTE-01`, `COACH-REVIEW-2026-09-23`, `RUBRIC-STALE-BAR-01`, `DB-USER-PURGE-01`.
