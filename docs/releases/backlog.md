...
**State at END of 2026-09-25 (last ship `4a5d3c3f`, `NAV-SLIM-01`). 3,568 tests / 402 files. Docs ALL CLEAN.** 🔴 **THE DAY'S PATTERN IS POPULATION, NOT PREDICATE — four gates read clean while pointed at incomplete sets, all mine, all written this same week.** `sheetClose`'s `CONSUMERS` was four hand-typed paths picked because their FILENAMES say "Sheet", so it never looked at `DashboardClient`, **which renders four of the nine sheets** — the founder found the double cross by tapping it. The 44px floor arm measures only controls **already on the system**, i.e. not the population that violates it (**18 hand-rolled under 44px, smallest 18px**). The geometry baseline was keyed `file:line`, so one inserted line re-keyed everything below and it printed `moved: 0` — **not "nothing moved", no longer looking** (**145 of 145 orphaned**). And `buttonOwnership`'s filled-control arm anchored `color:` before the quote, so **a CTA with a disabled state — i.e. every real CTA — was invisible**; widened, **nine live at 3.68:1** incl. the wizard's Next. **Now a catalogue class in `/zona-debug`, the INVERSE of "checker reads a different source": there the value is wrong, here the value is right and the SET is short, which is worse — a short set produces a green tick.** 🟢 **`SWITCH-PRIMITIVE-01`** — `ui-patterns.md` described a 44×26 toggle in four bullets with **nothing implementing it**, while a thousand lines below it said *"never build a one-off toggle inline"*: **the document forbade the only available method.** **New rule: a section that names a control must name the component that implements it, or it is a sketch.** 🟢 **`NAV-SLIM-01`** — founder asked for a slimmer nav *"many times"*; **§ 7 has said 60px and 0.6875rem for months** and the bar shipped **64px**, because `.btn--regular`'s **44px CTA floor governed the chrome** (same property as the Switch, same evening). ⚠️ **The fix was not −4px:** the bar padded 10px around a 44px button, so **44 of 64 = 69% was tappable**; padding moved into the tab → measured in a real DOM **61px bar, 60×94 tab, 98% tappable — bar −4, target +16.** 🥇 **THE CONSUMER CHECK INVERTED MY PREMISE: `PhoneShell.NAV_H = 60` — the WEBSITE was right and the APP had drifted**, so slimming CLOSED a divergence. And `GuideSheet`'s mirror listed **`strava`** (tab retired in Phase 1), omitting **`me`** — one `NAV_ITEMS` now feeds both. 🔴 **One falsification reported 0 failures and that was MY HARNESS** — the mutation left `</div>` against `<button>`, so the file never loaded and `grep -c '×'` cannot see a suite that fails to IMPORT: **a hollow check inside the tool for detecting hollow checks.** 🟡 **`NAV-FLOAT-01` deferred, NOT killed** — `/nav-preview` gives the founder both navs over the same scrolling content. 📐 **159 controls · geometry moved 0 · under floor 0 · build 0.** ⚠️ **Every defect today he found on his phone; my gates found none of them first.**
