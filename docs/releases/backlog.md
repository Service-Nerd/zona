...
**State at END of 2026-09-20 (last ship `7ba000b`):** tree clean · `verify` exit 0 · **2,538 tests / 288 files** · **124 invariants** · sweep 14,189 plans no new violations · `audit-docs.sh` ALL CLEAN · **`measure:envelope` UNCHANGED on every distance, whole product 95.9%** (5K/10K/50K/100K 100%, HM 96.2%, marathon 90.1%). **TEN SHIPS today off the Miles teardown:** §80 Am.2 (`MARA-LR-LOWBASE-01`) · `REFRAME-NOTE-LOSS-01` · `P-13a/b` · `P-03` · `P-01` · `ENRICH-PII-MINIMISE-01` · `LEGAL-PRIVACY-01` · `CONSENT-DISCLOSURE-01` · the SLT copy rulings (**two of my three live strings were WRONG and are fixed**). **Also closed:** `STRAVA-APP-INACTIVE-01` — founder decision, we are not using Strava; the killed-app auto-link dies with it, permanently. **58 open, categorised in `docs/BACKLOG-BY-CATEGORY.md`** — brand/voice/copy **0**, live defects **0**. ⚠️ **The 95.9% was proven unmoved after every engine change, not asserted.** ⚠️ **Nothing has run on a device.**
