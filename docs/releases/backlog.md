...
> | 🔴 `S106-FLAT-PEAK-01` | **P1** | **Coaching Board 2026-09-18 — filing this is a CONDITION of the §111 Am.1 ruling, carried by Hutchinson.** MEASURED: a beginner marathon plan delivers a **flat 59 km/week peak for every runner from 16 to 60 km/week** (`PEAK_KM_BY_LEVEL.MARATHON.beginner` is 52; §23's `PEAK_OVER_BASE_RATIO` floor appears to lift it). Seiler: *"telling a runner on 16 and a runner on 60 to arrive at the same place is not a coaching decision, it is the absence of one."* McMillan: the gate is the wrong thing to argue about while the DESTINATION is a constant. ⚠️ **Hutchinson refused to authorise a `PEAK_KM_BY_LEVEL` change in the same sitting as a gate fix, on blast-radius grounds** — this needs its own sitting, touching §23, §106 and `PEAK_KM_BY_LEVEL` |
