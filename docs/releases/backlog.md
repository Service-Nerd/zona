...
`DashboardClient.tsx:3171`:

```
padding: 16px · background: var(--moss) · color: var(--card) · border: none
borderRadius: var(--radius-lg) · fontSize: 13px · letterSpacing: 0.08em
textTransform: uppercase · cursor: pointer · fontWeight: 600
```

Nine lines describing what a green button looks like, **written out 65 times**. Nobody ever made a
button decision; they made it 65 times from whatever was nearby. That is why the buttons look flat
**and** slightly different from each other.

| | |
|---|---|
| Hand-rolled moss-background elements | **65** |
| Shared `Button` component | **none** (`BackButton` is the only one in `components/ui`) |
| `--shadow-card` consumers | **19** |
| **`--shadow-lifted` consumers** | **0** — authored for the raised state, never used |
| The only CTA class | `.cta-pill`, used by **4 files**, a hover background swap. It declares `transition: transform` and **nothing ever uses it** |

**The product elevates its CARDS and gives its BUTTONS nothing**, so a moss button sits visually
*behind* the card it is on.

#### The sitting that already happened (2026-09-24)

Ruled **SHIP WITH AMENDMENT (4)** — recorded in `design-rulings.md`, **NOT built** at the founder's
request. Its substance stands as input to the mock-up round, not as a settled answer:
one Button component · primary gets `--shadow-lifted` · a press state that moves 1px · **email gets
a 1px border, because Outlook drops `box-shadow`** · 44px minimum height.

⚠️ **Collins won the framing and it is recorded:** the item was filed as *"does the CTA earn
elevation"* and the answer is that **the missing component is the cause; elevation is one of its
properties.**

#### What the founder wants before any build

1. **Mock-ups.** Proposals of what is right for the brand, seen before a ruling binds.
2. **A no-chrome revisit, WITH EXAMPLES.** The rule reads *"No chrome. No stacked shadows, no
   gradient on gradient, no decorative dividers"* — STANDING, Design Board, transferred 2026-09-22.
   ⚠️ **It is more permissive than it sounds and that matters here:** it bans *stacked* shadows, and
   both existing elevation tokens are themselves two-layer definitions, so a single named token is
   not what "stacked" means. The board should bring **worked examples of what it does and does not
   forbid**, because the rule has already been over-read once — in the email sitting this morning,
   where *"no decoration"* was taken to mean *no visual system* and had to be corrected by its own
   author (*"restraint is a hierarchy decision, not an absence of one"*).

#### Constraints any proposal must respect

- 🔴 **"Elevation is `--shadow-card` / `--shadow-lifted`. Do not design a new elevation system."**
  STANDING (P-13 correction). **A button-specific shadow token is already forbidden.**
- **Email cannot use `box-shadow`** — Outlook drops it. The two surfaces must stay recognisably the
  same button by some other device.
- **W-11 stands**: the paper-grain overlay was killed unanimously by the SLT (2026-09-21, Wood's
  kill mandate) because it changed no decision and *"the sole argument for it was that a competitor
  has it"*. Re-proposed the next day and declined again. **Reopening it needs that rule overturned
  by name, not an exception.**
- **"Type accent, not flood"** constrains any answer reaching for more colour.
- Sierra's bound: a button that feels better made flatters the app, not the runner. **Depth, a
  press state, a consistent size — the moment it becomes bounce or glow it has crossed over.**
...
anyway. That is how this codebase is already retiring the legacy colour aliases, and it has worked.
