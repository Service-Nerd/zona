...
### ⚙️ `PLAN-LEGACY-ZONE-STRING-01` — pre-§84 plans carry a zone string their target contradicts
**Board: ⚙️ NO BOARD** (a defect fix restoring documented intent) — **🏃 COACHING BOARD only if
the repair would change what any session prescribes.**
Found 2026-09-24 while repairing `PLAN-RESTING-HR-ZERO-01`. **Separated deliberately: it is the
residue that remained after that fix, and it is a different defect.**

Three live plans still hold `zone: "Zone 3–4"` on quality sessions whose `hr_target` covers
**Zone 3 only** — e.g. `939a5ada` w? reads `Zone 3–4 = 140–163 bpm` in the header against
`hr_target 140–152 bpm`. That is the ORIGINAL §84 defect (header and note disagreeing), on plans
generated **before the 2026-09-04 pairing fix**, and it is why their `DISPLAY-ZONE` counts did
not move when the resting-HR key was stripped (32 → 32, 25 → 25, 40 → 40).

⚠️ **THE RESTING-HR FIX WAS NOT EXPECTED TO CLEAR THESE AND DID NOT.** Saying so is the point:
...
a second founder-directed exception, or whether they age out. **Nothing prescribed is wrong** —
only the label above it.
