...
**State at END of 2026-09-25 (last ship `89d5a3cc`, `BUTTON-MIGRATION-03`). 3,552 tests / 400 files. Docs ALL CLEAN.** 🔴 **THE CHECK WRITTEN TO FIND PRIMARY CTAs WAS BLIND TO PRIMARY CTAs.** `buttonOwnership`'s filled-control arm anchored its quote directly after the colon — and a REAL primary CTA expresses a disabled state, so its colours are **ternaries**. The founder said *"there is also a save button at the bottom"*; I never checked it, it was hand-rolled, and the gate was green over it **twice**: `--teal` is a legacy ALIAS of `--moss`, and its label colour was conditional. Widened, it found **NINE more live CTAs at 3.68:1**, including the **wizard's Next** and **Use this plan**. 🔴 **WORSE: THE GEOMETRY BASELINE HAD STOPPED MEASURING.** Keyed `file:line:tag`, and the comparison skips a key the baseline lacks — so **inserting one line re-keyed every control below it, all were skipped, and the gate printed `moved: 0`.** Not "nothing moved": **no longer looking.** Falsified on the real incident — **145 of 145 orphaned while the move arm stayed green.** Re-keyed by ordinal + a coverage arm that fails on orphaning. 🔻 **`TAP-TARGET-FLOOR-01`: the 44px floor arm measures only controls ALREADY ON THE SYSTEM** — i.e. not the population likely to violate it. Filter dropped: **18 hand-rolled controls under 44px, smallest 18px.** 3 fixed here incidentally; **15 remain**, each needing a per-control decision (convert, or inline target), never a blanket height. ⚠️ **Two excluded BY NAME with reasons, not restyled** — the injury chip's moss is `ui-patterns.md`'s only selected affordance, the banner's amber is ADR-012 semantics; folded into `DANGER-TEXT-CONTRAST-01` for the board. 📐 **157 controls · geometry moved 0 · below the floor 0.** ⚠️ **Still nothing verified on a device.**
