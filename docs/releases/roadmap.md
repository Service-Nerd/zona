...
| **#7b** | **HR-SYNC-03** — Swift HealthKit bridge (Layer 2) | 🔄 | M (~3–5 days) | **Code shipped 2026-06-24. Needs Xcode build + TestFlight.** `HealthObserverPlugin.swift/.m` created. TS binding `lib/health/observer.ts`. Wired into `CapacitorBoot.tsx`. `packageClassList` updated. Pending: Xcode target membership + cap sync + archive. |
