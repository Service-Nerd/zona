...
> | 🔴 `S111-SUBFLOOR-VOLUME-01` · `S111-DENOMINATOR-01` | Ruled 2026-09-19. **Their blocker `S52-LOPSIDED-BOUND-01` is now CLOSED**, and the closure is a NEGATIVE result: the share cannot be driven down by shortening the long run. `S111-SUBFLOOR-VOLUME-01` therefore stays where the SLT left it (ask the charity first); `S111-DENOMINATOR-01` stays sequenced behind a peak change. |
...
> | 🔵 `S9-DURATION-FLOOR-01` | **NEW 2026-09-19, P2, Coaching Board.** §9's minimum-session-size floor is in KM and `INV-PLAN-MIN-SESSION-SIZE` skips duration-anchored sessions, so it reaches **no beginner session at all**, quality or easy. Pre-dates §110 Am.2. ⚠️ **The obvious fix was tried and is WRONG** — it fires on ordinary beginner easy runs (30 min at beginner pace = 3.9 km vs a 4 km floor). §9 needs a MINUTES equivalent, which is a coaching decision. |
> | 🔵 `S53-PIGEONHOLE-ARM-01` | **NEW 2026-09-19, P3.** 2 plans in 14,486: five quality picks from a pool of two rows, so the cap of 2 is arithmetically unsatisfiable — the pigeonhole arm computes 2 where `ceil(5/2) = 3`. Baselined with the reason. ⚠️ **Do not widen the pool to fix it** — the row that would is rung 5, which the board DEFERRED. |
