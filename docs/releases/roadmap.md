...
| **RAMP-BOUNCEBACK-01** — bound the post-deload bounceback | FREE | S | **P1 — a live §12 gap, not just a blocker.** §2 exempts a post-deload bounceback from the injury cap but does not BOUND it, so a **+26% week already passes today** on the knee archetype. Whether a 30% jump is legal depends on where the phase boundary falls. Blocks the ruled base 35→30. Needs a board sitting (Willy owns §2/§12). |
| **CB-PHASE-01 re-take** — base 35→30 | FREE | XS | Ruled CORRECT 2026-09-04, built, **reverted** — blocked behind RAMP-BOUNCEBACK-01. Gains HM/MAR/50K a week of build; 5K/10K unaffected. **Do not re-propose 25%** (declined by Seiler and Willy on the volume ramp). |
