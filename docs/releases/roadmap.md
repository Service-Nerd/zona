...
| **SESSION-KM-02's last two sites** (§47 long-run alternation, §52 easy floor) | Still held, but **the blocker is discharged: the detector was built and the impact IS now measured (2026-09-12)** — and it overturned the item. **The filed fix is a provable no-op**: both `?? 0` swaps changed ZERO plans across 2,916 parity cases and all 648 cohort plans, because a THIRD gate sits inside the mutation. The real question is whether a beginner's step-back is expressed in km or minutes, which is bigger than what was filed and still needs the board. Brief: `docs/investigations/session-km-02-measurement-2026-09-12.md`. |
