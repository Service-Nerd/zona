...
**Tree clean, 3 unpushed** (last ship `f7af9de`, 2026-09-18). `npm run verify` exit 0 (**2,296 tests / 254 files**) · `check:db` exit 0, and it no longer writes 43 errors into the log view per healthy run · sweep 13,014 plans, **0 hard failures**, no new violations · HIGH 0. **Seven items shipped in one autonomous run:** `PREF-SWEEP-01` (the unit preference reaches the last five surfaces, + a gate) · `FATIGUE-ARRAY-DRY-01` (seven copies, not five, and the sharpest was a TYPE) · `OPS-DBCHECK-NOISE-01` (the drift check reads the schema; the noise was the smaller half) · `CI-SLOW-DRIFT-01` (`npm run check:slow`) · `REFUSAL-COPY-02` (and the discovery that the refusal COPY WAS A WIRE FORMAT in eight places) · `FIRSTRUN-GATE-CALL-01` (SLT ruled; card ungated, sentence gated, original wording blocked as a capability claim) · `GRID-SUBFLOOR-01` (resolved as DO NOT ADD, with the numbers). ⚠️ **Three findings came from checks catching ME mid-edit, not history** — the new units gate caught my own unreachable fallback, and I twice built measurements on inputs that cannot occur (`distance_km ?? 0` on a duration-anchored plan; `cwk 40 / longest 4`) and was drafting false findings from both. **Nothing here has run on iOS** (`DEVICE-VERIFY-01`, parked). **Post-run review (same day):** the coaching round ran clean (21 cases, 0 errors, M4 refused by design) and the **Coaching Board ruled CORRECT** — no artifacts, because the constitution was already right. ⚠️ **The sitting withdrew its own question (a):** M1/M1d's 26 km peak long run is not a shortfall, it is `LONG_RUN_CAP_MINUTES` binding at 3h 28, and §80 governs DURATION not distance. It also found a registry defect (`INV-PLAN-LR-MAX-WEEKLY-PCT` is `warn` when `volume_profile` is maintenance, `error` otherwise; the doc said only `error`) and filed `BRAND-MAINT-LABEL-01` (SLT) + `S80-MATERIALITY-EVIDENCE-01`. **Parity vs the last pre-today commit: all 2,007 changed cases attributed, 0 unexplained, `REFUSED->OK` = 0** (`coaching-review/2026-09-18/parity-attribution.md`). **Contracts:** two the audit could not see are fixed and its `route.ts`-only regex is widened to `.tsx`. **`COMPONENT-CONTRACT-GATE-01` shipped (last ship `569c3c9`):** `docs/contracts/components/` is checked now, by a prop-name test in both directions plus the mirror of the API touch-check. ⚠️ **It found that `session-card.md` documented SEVEN props of which NOT ONE EXISTED**, while all fifteen real ones went undocumented — a contract describing a component that was never built, reading as authority the whole time. Each contract now declares its own `**Component:**`, so the checker does not hold the list it checks. 🔴 **`REFUSAL-ALT-REACHABLE-01` (last ship `38df833`) — the §113 refusal offered a door that was also LOCKED.** It told a refused first-time marathoner *"start with a half marathon plan, which asks less of your longest run"*; the half carries the **same** 5 km floor and refuses them identically (measured: marathon REFUSED, half REFUSED, 10K generates). Now offers the 10K. Found only by **executing the advice**, and the existing §113 test had PINNED the defect. 🟢 **`CB-SUBFLOOR-ADMIT-01` shipped (last ship `5461d35`) — the charity cohort is no longer refused at the door.** §113 Am.1: the 5 km floor was applied AFTER §45's week-1 cap and overrode it, turning a **+10% opening week into +67%**, and §113 then refused the runner for the leap the floor had created. Floors now resolve per runner; §113 is runway-aware. **Sweep generates 1,216 MORE plans (13,014 → 14,230), refusals 6,993 → 5,777.** ⚠️ **§111 still refuses below ~16 km/wk** — separate gate, not ruled on. ⚠️ **No standing harness sees this cohort** (0 of 37,248 grid rows), so parity/cohort:shape/measure:fitness all read NO CHANGE. Follow-up `f7af9de`: the **foundation block was still handing the sub-floor runner the 5 km leap** (+67%) in week -1, before week 1 — migrated. 4 more items filed, incl. 🔴 **`S111-SUBFLOOR-VOLUME-01` (P0): §111 still refuses below ~16 km/wk, so §113 alone did NOT clear the door.**
