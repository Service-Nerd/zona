...
**State at END of 2026-09-19 (last ship `baf2ebb`):** ✅ tree clean. `npm run verify` green — **2,341 tests / 261 files** · **121 invariants** (121↔121) · matrix 65/0 · sweep **14,476 plans, no new violations** · `invariant:liveness` **118/121 (98%)** · `audit:plans` **now GATED in verify**, no regression · `audit-docs.sh` ALL CLEAN. **13 commits.**

🔴 **THE ENGINE IS NOT FIT FOR PURPOSE FOR PRIORITY ONE, AND IT IS NOW MEASURED RATHER THAN SUSPECTED.** `npm run audit:plans` over 6,480 beginner-marathon inputs: **36.4% refused outright**; of those built, **49.4% short of declared days · 45.2% with a ≥75% single session · 40.5% with ≤2-run weeks · 32.8% with a week-1 leap >30%**. **83% regress the midweek runs when the build begins** (long run 5.3→26.0 km while Mon/Wed/Fri stay 3.9 km from week 1 to 13). 14 volumes → 6 distinct plans.

🔴 **TEN INSTRUMENTS BUILT AND MEASURED, ALL TEN TRADED ONE DEFECT FOR ANOTHER** (full table in §9's *Recorded structural finding* — **do not retry blind**). The cause is one sentence: **§45's cap is multiplicative on the previous week's long run, so reducing any week ratchets the trajectory down and it never recovers.** Bounding at construction rather than as a post-pass fixes composition completely (binge 45.2→0%, degenerate 40.5→0%) and guts specificity, because **the binge weeks ARE the specificity floor** — flooring the bound at it makes the bound inert. **Remedy is architectural: size the long run and the week TOGETHER in `buildWeekSessions`.** Highest-value open engine item.

🟢 **Shipped 2026-09-19:** `CB-HILL-INJURY-01` (§28 Am.2 — **a live safety defect: knee-history beginners were prescribed hill strides**; §21's invariant matches the LABEL and the label lied. **Found by making a label honest, not by measuring**) · `STRIDE-VISIBILITY-01` · `TAPER-FLOOR-FLAT-01` (38.8% of taper weeks) · `MAINT-LIVENESS-01` (**the harness was calling the wrong validator** — liveness 107→118/121, §67/§75 coverage debt discharged) · `STEPBACK-STALE-PEAK-01` · `GRID-MARATHON-CAPABLE-01` (the grid **could not contain a §24-capable marathoner**; §24 now reached by 68 rows vs 0) · `PLAN-QUALITY-AUDIT-01` (gated) · `PLAN-NOTE-LENGTH-01` · `INV-MSG-ROUNDING-01` · `XREF-DANGLE-01`.

🟢 **Closed by measurement:** `S80-MATERIALITY` (bimodal, **exactly empty 2–5% band** — 5% correct) · `BEGINNER-5K-QUALITY-01` (0% lack neuromuscular) · `S28-CAP-ORDER-01` (**filed mechanism does not exist**) · `S45-ABS-STEP-01` (harm > benefit) · `QUALITY-FLOOR-SUBFLOOR-01` · `BRAND-MAINT-LABEL-01` · `LR-CONSEC-01` (superseded by §9's finding). `PLAN-RUNWAY-CHARITY-01` **answered: SIX WEEKS UNCOVERED** for every charity runner.

🔴 **OPEN, in priority order:** the **§9 architectural fix** · `PEAK-VS-DELIVERED-BUILD-01` (**new — 47.8% of experienced 70 km/week marathoners get a plan whose DELIVERED peak is under §23's 110%**; the curve satisfies it by construction, so it is the curve-vs-delivered gap again) · `S111` family + `S52` (all subordinate to the architectural fix) · `DEVICE-VERIFY-01` (founder's phone) · SLT BUILD items that are wizard/marketing, not engine.
