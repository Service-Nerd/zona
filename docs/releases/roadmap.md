...
**Last updated:** 2026-08-06 (**Wave 1b opened — plan generator remediation, GEN-FIX-00…08.** First organic user's plan carries 3 P0 defects; root-cause analysis + SLT review in `docs/incidents/2026-08-06-plan-defects/analysis.md`. Takes the front of the Product queue; **acquisition paused** until GEN-FIX-02→05 land. Four coaching-rule changes board-approved (D1–D4); D5 promoted to a P0 build. Also: W2 closed — MON-TRIAL-01 resolved 2026-08-06.)
...
> 🔴 **2026-08-06 — acquisition on hold (GEN-FIX-00).** The first organic user's plan carried three P0 generator defects. Until GEN-FIX-02→05 land, every user acquired is burned *and* contaminates the INSTRUMENT-01 baseline — which is the only measurement we have. This is a deliberate, reversible hold on ASO push / community seeding / waitlist sends, not a change of strategy. Lift when GEN-FIX-05 is merged and the golden personas are green.
...
> 🔴 **Wave 1b — plan generator remediation (GEN-FIX-00…08) takes the front of this queue.** The first organic user's plan was reproduced byte-for-byte from the live engine and carries three P0 defects: it **ends 11 days before their race** (universal — all 7 simulated personas end 3–24 days early), **HR zones are 28 bpm low** (the wizard reads Apple Health's highest-ever sample as physiological max), and **beginners get copy promising quality sessions the engine cannot produce**. Root causes and evidence: `docs/incidents/2026-08-06-plan-defects/analysis.md`; specs in `backlog.md § Plan generator remediation`. **GEN-FIX-00 pauses acquisition until GEN-FIX-02→05 land** — every user acquired meanwhile is burned *and* contaminates the INSTRUMENT-01 baseline.

| Priority | Item | Status | Effort | Notes |
|---|------|--------|--------|-------|
| **0a** | **GEN-FIX-00** — pause acquisition | 🔲 | XS | Not code. Verify a tap exists first — no paid spend believed live, so this may cost nothing. Revisit trigger: GEN-FIX-05 merged + golden personas green. |
| **0b** | **GEN-FIX-01** — User A remediation | 🔲 | XS + S | Stage 1 **today, no build**: correct `user_settings.max_hr` → 178; displayed targets recompute live. Founder-sent, one question, no trial comp. Stage 2 after 03 + 06 — **~2-week clock**. |
| **0c** | **GEN-FIX-02** — enrichment observability | 🔲 | XS | Trial plan shipped with zero AI and nobody knew for 5 days. Silent failure is indistinguishable from "didn't convert" — corrupts the funnel diagnosis. N-015 class, recurring after W1. |
| **0d** | **GEN-FIX-03** — plan must reach race day | 🔲 | XS + S | Invariant first, watch it fail, then fix. Lay weeks backward from race week. Isolated, no cascade. |
| **0e** | **GEN-FIX-04** — real 5K time trial in recalibration weeks | 🔲 | S | Board-promoted 9th → 5th, unanimous. Only exit from the HR loop; also the contrast case that makes zone discipline learnable. |
| **0f** | **GEN-FIX-05** — HR plausibility gate | 🔲 | S + M | Guaranteed to be most wrong for the ideal customer. Run the population query in the same pass. |
| **0g** | **GEN-FIX-06/07/08** — copy-reads-sessions · 4 rule changes · regression harness + governance | 🔲 | M each | 07 runs parallel (config + canon only). 08 includes the "rule about rules" and the feature-registry correction. |
