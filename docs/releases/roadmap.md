...
**State at end of 2026-09-19 (last ship `1713df9`):** `npm run verify` exit 0 — **2,391 tests / 268 files** · **122 invariants** · sweep **14,490 plans, 0 hard failures, no NEW violations** · `audit-docs.sh` ALL CLEAN. **Engine backlog closed.** Last ship fixed `QUALITY-AERO-FALLBACK-01` — 144 quality sessions were a Z2 aerobic row labelled as pace work, **100% of them in the priority-one cohort**, introduced that morning by §110 Am.2 opening the beginner quality slot. `S53-ROTATION-SCARCITY-01` **closed by SLT ruling: DON'T BUILD** (4 fixes measured and reverted; the 2 breaches stay baselined with the true cause).
...
> | 🔴 `S111-DENOMINATOR-01` | 🔴 **RE-MEASURED, RE-HELD 2026-09-19 — and my re-measurement was WRONG first.** The recorded blocker was "+624 refusals, +48%". Re-measuring on `cohortGrid + targetedGrid` gave **§111 refuses 0 today, 112 with the fix (0.24%)**, so I concluded the blocker had evaporated and shipped it. `audit:plans` then failed on its own corpus: **BEGINNER MARATHON (priority one) generated 4,159 → 3,703, refused 2,321 → 2,777 (+456, +19.6%)**. ⚠️ **The 6,480-input beginner-marathon corpus is in NEITHER grid I measured on.** I measured the cohort the fix is safe for and missed the one it endangers. Reverted; the hold stands with the correct number. **What releases it:** a change that lifts low-base beginners' delivered peak so the real ratio falls under the cap — NOT another re-measurement of this fix alone. |
