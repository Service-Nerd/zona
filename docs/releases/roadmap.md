...
| Item | Status | Effort | Notes |
|------|--------|--------|-------|
| **Reactivate the Strava application** (STRAVA-WEBHOOK-OBS-01) | 🔴 | XS (external) | **Founder action at Strava, not fixable from the repo — and it is live now.** Measured 2026-09-13 with production credentials: `403 { resource: 'Application', field: 'Status', code: 'Inactive' }`. Not a missing subscription — in that state **none can exist or deliver**, so the killed-app auto-link is dead, which is exactly the reported "runs only link when I open the app". A fully-killed iOS app cannot background-ingest from HealthKit, so the webhook was the only cover for that case. Likely tied to the pending Strava API approval. ⚠️ The local `.env.local` secret is corrupted and returns **401**, pointing at credentials instead of at the application — do not diagnose locally. The daily probe now records this state with the remedy attached, so it can no longer be silent. |
