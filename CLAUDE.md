...
- `npm run invariant:liveness` — **"can each rule still be made to FAIL?"** `verify:invariants` proves every registered invariant (**115** as of 2026-09-15) is registered, declared and do not FALSE-fire on three canonical cases. It says nothing about the opposite and more important property: that a rule can fire **at all**. This repo has shipped a green tick with nothing behind it more than once (`--section-gap`, the decorative-config family, D9's `flexShrink` that could never fire, §97's two inert gates). ⚠️ **The obvious measurement is the wrong one:** when this shipped, 86 of the then-93 invariants never fired across 621 generated plans, and that is exactly what a HEALTHY engine looks like — an invariant is meant to be silent, so silence cannot tell a working rule from a dead one. So the harness **deliberately breaks valid plans** (**66** mutations as of 2026-09-16: strip a field, invert the volume curve, make every week a peak, make the long run 90% of the week) and records which rules wake up. A rule nothing wakes is **UNPROVEN, not proven dead**, and lands in `__fixtures__/invariantLivenessBaseline.json` with a reason — `corpus` (the harness never builds that plan SHAPE — maintenance has its own generator; foundation / recalibration / ultra are outside the cohort grid), `mutation` (the battery does not reach that field yet), or `unclassified` (**the column that should shrink**). Enforced by `invariantLiveness.test.ts` inside `npm run verify`: a NEW invariant nothing can wake fails the build, so the next one proves itself on the way in. Same debt-register pattern as SWEEP-BASELINE-01. Delivered 2026-09-11 as test-coverage proposal #5.
