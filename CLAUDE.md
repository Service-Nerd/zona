...
- **Three stores no FK can reach** are cleared by the `on_auth_user_deleted` trigger: `ai_rate_limits` (keyed `ai:<surface>:<uuid>` in TEXT), `charity_codes.claimed_at` (or the batch cap stays spent on a runner who is gone), `waitlist` (email-keyed).
