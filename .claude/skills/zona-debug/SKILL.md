...
| **Orphaned by unioned exclusions** | Several guards each decline the case for a *defensible* reason; no guard is wrong; the case has zero coverage and nothing measures coverage | `hill_reps` was declined by `pacedRepPlan` (no pace on the work step), `pacedRepMainMinutes` (landmark/open lengths → "skip rather than guess") and `INV-PLAN-VO2MAX-MAIN-SET-CAP` (no `pace_target`) simultaneously — so the only effort-governed row in the catalogue had no sizing and no duration check at all, for as long as it existed |
| **Checker reads a different source from the producer** | A guard and the thing it guards derive the same quantity from different fields, so the guard is measuring something else. Passes or fires for reasons unrelated to the defect | Three times on 2026-09-04 alone: §84's header read `session.zone` while the coach note read `hr_target`; a new invariant read `input.max_hr` while the engine used §50's *guarded* max and fired 11,556 times; `pacedRepPlan`'s distance floor divided by work pace while the caller priced segments separately, certifying a 4.5 km session as clearing a 5 km floor. **A floor measured in different units from the thing it guards is not a floor.** |
| **Hazard solved for one transition, named but not solved for its twin** | An ADR identifies a structural hazard, solves it for the case in front of it, documents the SIBLING case in the same file, and the solution is never carried across. Tell: a lifecycle diagram or state list where one arrow is defended in prose and an adjacent one is not | ADR-013 §22–26 solved `week_n` collisions for race → maintenance by continuing the sequence at 26+ (*"never collide … No migration"*), and §36 names `user generates next race` in the same lifecycle — the arrow that restarts numbering at 1. A new 12-week plan arrived **94% pre-completed**, five sessions linked to runs five months old. Its Follow-ons had even considered and REJECTED `plan_id`-scoped completions — correct for the transition being designed, and that rejection then covered one nobody re-examined. **When an ADR solves a hazard, check every transition it names, not the one it was written for.** (PLAN-WEEK-COLLISION-01, 2026-09-18) |
