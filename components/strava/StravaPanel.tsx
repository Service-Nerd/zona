...
export default function StravaPanel() {
  ...
  async function connect() {
    ...
    try {
      ...
      const tokenRes = await fetch('https://www.strava.com/oauth/token', {
        ...
        body: JSON.stringify({ client_id: CLIENT_ID, client_secret: secret, refresh_token: REFRESH_TOKEN, grant_type: 'refresh_token' }),
      })
      const { access_token } = await tokenRes.json()
      if (!access_token) throw new Error('No token returned')

      sessionStorage.setItem('strava_secret', secret)
      ...
      setRuns(sorted)
      setConnected(true)
      checkForNewActivity(sorted, secret)
    } catch (e: unknown) {
  ...
  function checkForNewActivity(runs: StravaActivity[], _secret: string) {
...
Give 3-4 sentences of direct coaching feedback. Flag if HR was too high. Note one thing done well and one to focus on next session. No fluff.`
  ...
  const avgHR   = runs?.length
    ? Math.round(runs.filter(r => r.average_heartrate).slice(0, 10).reduce((s, r) => s + (r.average_heartrate ?? 0), 0) / Math.min(runs.filter(r => r.average_heartrate).length, 10))
    : null
  ...
  return (
    <div>
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '6px', padding: '16px 18px', marginBottom: '16px', fontSize: '0.84rem', lineHeight: 1.75, color: 'var(--text-dim)' }}>
        Your Strava data — pulled live. Enter your Client Secret once per session. <span style={{ color: 'var(--orange)' }}>New activities trigger an AI coaching popup on connect.</span>
      </div>
      ...
      {!connected && (
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '18px', flexWrap: 'wrap' }}>
          ...
          <button onClick={connect} disabled={loading || !secret}
            style={{ background: 'var(--orange)', color: 'var(--black)', border: 'none', borderRadius: '5px', padding: '10px 20px', fontFamily: "'DM Mono',monospace", fontSize: '0.72rem', letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 'bold', opacity: loading ? 0.6 : 1 }}>
      ...
      {connected && <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '0.65rem', color: 'var(--green)', marginBottom: '16px' }}>✓ Strava connected</div>}
        ...
        <>
          ...
          <div style={{ display: 'grid', gap: '6px' }}>
            {runs.slice(0, 12).map(run => {
              ...
              return (
                <div key={run.id} style={{ display: 'grid', gridTemplateColumns: '60px 48px 64px 72px 72px 50px 1fr', gap: '8px', padding: '9px 14px', background: 'var(--card2)', border: '1px solid var(--border)', borderRadius: '5px', alignItems: 'center', fontSize: '0.8rem' }}>
