...
export function validatePlan(plan: Plan, rawInput: GeneratorInput): Violation[] {
  ...
  for (const w of plan.weeks) {
    ...
    // ⚠️ MOVED OUT OF THE PER-WEEK LOOP (2026-09-21). This is a PLAN-LEVEL
    // check — it builds `phaseOfWeek` from `plan.weeks` and iterates them
    // itself — but it sat inside `for (const w of plan.weeks)` opened ~760
    // lines above, so it ran once PER WEEK and a single offence emitted one
    // identical row per week in the plan (sixteen rows for one bad deload on
    // a 16-week plan). It closes over `plan` and `input` only, never the loop
    // variable, so relocating it is behaviour-preserving for the violation SET.
    //
    // Fixed now rather than later because it is currently FREE: measured
    // across the full 45,776-plan corpus it fires ZERO times, so no debt
    // register, sweep baseline or violation count moves. It is also absent
    // from `invariantLivenessBaseline.json`, i.e. the liveness harness has
    // PROVEN it can be woken by mutation — so this is a live rule that simply
    // never fires on a healthy plan, which is what a healthy invariant looks
    // like. The duplication was latent, not harmless: it would have inflated
    // every count this check contributes to on the day it first fired.
    //
    // Found by the session working on MKT-PLAN-SHAPE-01, which hit the same
    // shape with a new invariant of its own and correctly moved theirs out.
